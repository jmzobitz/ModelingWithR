...
@tbl-limited-data-09 displays a dataset with a limited number of points where we wish to fit the function $y=bx$:
...
A small, limited dataset.
:::

For this example we are forcing the intercept term to equal zero - for most cases you will just fit the linear equation (see @exr-full-linear where you will consider the intercept $a$). @fig-quick-scatter-09 displays a quick scatterplot of these data:

```{r,warning=FALSE,message=FALSE,echo=FALSE,fig.cap="A scatterplot of a small, limited dataset (@tbl-limited-data-09)."}
#| label: fig-quick-scatter-09
...
Consider @fig-normal-shaded-09, which may be familiar to you as the normal distribution or the bell curve:

```{r,fig.cap='The standard normal distribution, with a shaded area between $x=\\pm 1$',echo=FALSE}
#| label: fig-normal-shaded-09
ggplot() +
  geom_area(
...
Now that we have made that small excursion into probability, let's return to the parameter estimation problem. Another way to phrase this problem is to examine the probability distribution of the model-data residual\index{residual!model-data} for each measurement $\epsilon_{i}$ (@eq-model-data-resid-09):

$$
\epsilon_{i} = y_{i} - f(x_{i},\vec{\alpha} ).
$$  {#eq-model-data-resid-09}

The approach with likelihood functions assumes a particular probability distribution on each residual. One common assumption is that the model-data residual is normally distributed. In most applications the mean of this distribution is zero ($\mu=0$) and the standard deviation $\sigma$ (which could be specified as measurement error, etc.). We formalize this assumption with a likelihood function $L$ in @eq-likelihood-resid-09.

$$
L(\epsilon_{i}) = \frac{1}{\sqrt{2 \pi} \sigma} e^{-\epsilon_{i}^{2} / 2 \sigma^{2} } 
$$ {#eq-likelihood-resid-09}

To extend this further across all measurements, we use the idea of *independent, identically distributed* measurements so the joint likelihood of **all** the residuals (each $\epsilon_{i}$) is the product of the individual likelihoods (@eq-likelihood-prod-09. The assumption of independent, identically distributed is a common one. As a note of caution you should always evaluate if this is a valid assumption for more advanced applications.

$$
L(\vec{\epsilon}) = \prod_{i=1}^{N} \frac{1}{\sqrt{2 \pi} \sigma} e^{-\epsilon_{i}^{2} / 2 \sigma^{2} } 
$$ {#eq-likelihood-prod-09}



We are making progress here; however to fully characterize the solution we need to specify the parameters $\vec{\alpha}$. A simple redefining of the likelihood function where we specify the measurements ($x$ and $y$) and parameters ($\vec{\alpha}$) is all we need (@eq-likelihood-prod-param-09).

$$
L(\vec{\alpha} | \vec{x},\vec{y} )= \prod_{i=1}^{N}  \frac{1}{\sqrt{2 \pi} \sigma} \exp(-(y_{i} - f(x_{i},\vec{\alpha} ))^{2} / 2 \sigma^{2} )
$$ {#eq-likelihood-prod-param-09}

Now with @eq-likelihood-prod-param-09 we have a function where the best parameter estimate is the one that optimizes the likelihood.

Returning to our original linear regression problem (@tbl-limited-data-09 and @fig-quick-scatter-09), we want to determine the $b$ for the function $y=bx$. @eq-small-data-likely then characterizes the likelihood of $b$, given the data $\vec{x}$ and $\vec{y}$:

$$
L(b | \vec{x},\vec{y} ) = \left( \frac{1}{\sqrt{2 \pi} \sigma}\right)^{4} e^{-\frac{(3-b)^{2}}{2\sigma^{2}}} \cdot e^{-\frac{(5-2b)^{2}}{2\sigma^{2}}}  \cdot e^{-\frac{(4-4b)^{2}}{2\sigma^{2}}}  \cdot e^{-\frac{(10-4b)^{2}}{2\sigma^{2}}}
$$  {#eq-small-data-likely}

For the purposes of our argument here, we will assume $\sigma=1$. @fig-small-likelihood-plot shows a plot of the likelihood function $L(b | \vec{x},\vec{y} )$.

```{r,warning=FALSE,message=FALSE,echo=FALSE,fig.cap='The likelihood function (@eq-small-data-likely) for the small dataset (@tbl-limited-data-09), with the value of the maximum likelihood at $b=1.865$ marked with a vertical line.'}
#| label: fig-small-likelihood-plot
b <- seq(0, 5, 0.01)
...
Note that in @fig-small-likelihood-plot the values of $L(b | \vec{x},\vec{y} )$ on the vertical axis are really small. (This typically may be the case; see @exr-show-small-likely.) An alternative to the small numbers in $L(b)$ is to use the log-likelihood (@eq-loglikely):
...
In @exr-small-data-optimize) you will be working on how to transform the likelihood function $L(b)$ to the log-likelihood $\ln(L(b))$ and showing that @eq-small-data-likely is maximized at $b=1.865$. The data with the fitted line is shown in @fig-quick-scatter-fit-09.

```{r,warning=FALSE,message=FALSE,echo=FALSE,fig.cap="A scatterplot of a small dataset (@tbl-limited-data-09) with fitted line $y=1.865x$ from optimizing @eq-small-data-likely."}
#| label: fig-quick-scatter-fit-09
data.frame(x = c(1, 2, 4, 6), y = c(3, 5, 4, 10)) |>
  ggplot(aes(x = x, y = y)) +
  geom_point(size = 2, color = "red") +
  xlim(c(0, 10)) +
...
```{r, fig.cap="Scatterplot of *Sacchromyces* volume growing by itself in a container.",echo=FALSE}
#| label: fig-yeast-quick-09
### Make a quick ggplot of the data
...
@eq-gause-09 shows the solution to this differential equation, where the additional parameter $a$ can be found through application of the initial condition $y_{0}$. 

$$
y = \frac{K}{1+e^{a-bt}}
$$ {#eq-gause-09}

In @gause_experimental_1932 the value of $a$ was determined by solving the initial value problem $y(0)=0.45$. In @exr-solve-gause you will show that $\displaystyle a = \ln \left( \frac{K}{0.45} - 1 \right)$. @eq-gause-09 then has two parameters: $K$ and $b$. Here we are going to explore the likelihood function to try to determine the best set of values for the two parameters $K$ and $b$ using a function in the `demodelr` package called `compute_likelihood`. Inputs to the `compute_likelihood` function are the following:
...
First we will define the equation used to compute our model in the likelihood. As with the functions `euler` or `systems` in @sec-euler-04 we need to define this function:
```{r}
library(demodelr)
...
It is also important to visualize this likelihood function. For this dataset we have the two parameters $K$ and $b$, so the likelihood function will be a *likelihood surface*, rather than a two-dimensional plot. To visualize this in `R` we can use a contour diagram. @fig-gause-likely-plot displays this countour plot.

```{r,fig.cap="Likelihood surface and contour lines for the `yeast` dataset."}
#| label: fig-gause-likely-plot
...
In @fig-gause-likely-plot there appears to be a large region where the likelihood has the same value. (Admittedly I chose some broad parameter ranges for $K$ and $b$). We can refine that by producing a second contour plot that focuses in on parameters closer to the calculated optimum value at $K=13$ and $b=0.07$ (@fig-revised-gause-likelihood):

```{r,fig.cap="Zoomed in likelihood surface. for the `yeast` dataset. The computed location of the optimum value is shown as a red point.",echo=FALSE}
#| label: fig-revised-gause-likelihood
...
The reported values for $K$ (12.8) and $b$ (0.241) may be close to what was reported from @fig-gause-likely-plot. Notice that in @fig-revised-gause-likelihood I also added in the location of the optimum point with `geom_point()`.


As a final step, once you have settled on the value that optimizes the likelihood function, is to compare the optimized parameters against the data (@fig-gause-model-data-09):
...
```{r,fig.cap="Model and data comparison of the `yeast` dataset from maximum likelihood estimation.",echo = FALSE}
#| label: fig-gause-model-data-09
# Define the parameters and the times to evaluate:
my_params <- gause_likelihood_rev$opt_value
time <- seq(0, 60, length.out = 100)
...
- The `|>` is the [pipe](https://r4ds.hadley.nz/workflow-style.html#sec-pipes). This is a very useful command to help make code more readable!
...
And the rest of the plotting commands you should be used to. This activity focused on the likelihood function - @exr-log-likelihood-yeast has you repeat this analysis with the log-likelihood function.
...
::: {#exr-show-small-likely}
Compute the values of $L(b|\vec{x},\vec{y})$ and $\ln(L(b|\vec{x},\vec{y}))$ for each of the data points in @eq-small-data-likely when $b=1.865$ and $\sigma=1$. (This means that these 4 values would be multiplied or added when you compute the full likelihood function.) Explain if the likelihood or log-likelihood would be easier to calculate in instances when the number of observations is large. 
:::

::: {#exr-log-likelihood-yeast}
Visualize the likelihood function for the `yeast` dataset, but in this case report out and visualize the log-likelihood. (This means that you are setting the option `logLikely = TRUE` in the `compute_likelihood` function.)  Compare the log-likelihood surface to @fig-revised-gause-likelihood.
:::


::: {#exr-small-data-invest}
When we generated our plot of the likelihood function in @fig-small-likelihood-plot we assumed that $\sigma=1$ in Equation (9.6). For this exercise you will explore what happens in Equation (9.6) as $\sigma$ increases or decreases.


a. Use [desmos](www.desmos.com/calculator) or `R` to generate a plot of @eq-small-data-likely. What happens to the shape of the likelihood function as $\sigma$ increases?
...
::: {#exr-small-data-optimize}
Using @eq-small-data-likely with $\sigma = 1$:
...
d. Compare this likelihood estimate for $b$ to what was found in @fig-small-likelihood-plot. 
...
::: {#exr-gause-ml-09}
The values returned by the maximum likelihood estimate for @eq-gause-09 were a little different from those reported in @gause_experimental_1932:
