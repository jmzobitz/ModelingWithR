# Systems of Linear Differential Equations {#sec-linearsystems-15}

Here we delve into a deeper understanding of differential equations by examining long term stability of equilibrium solutions. As a first step, @sec-linearsystems-15 focuses on *linear* systems of differential equations, such as @eq-example-ch15:\index{differential equation!linear system}
...
@eq-example-ch15 is a linear system of differential equations \index{differential equation!linear system} because it does contain terms such as $y^{2}$ or $\sin(x)$ on the right hand side of the equation. This chapter focuses on visualizing the phase plane for linear systems and determining the equilibrium solutions. Let's get started!
...
Another way to express @eq-example-ch15 is with matrix notation:\index{matrix notation}
...
(Note: we can also use the prime notation ($x'$ or $y'$) to signify  $\displaystyle \frac{dx}{dt}$ or $\displaystyle \frac{dy}{dt}$.) We can also represent @eq-example-ch15a in a compact vector notation: $\displaystyle \frac{ d \vec{x} }{dt} = A \vec{x}$, where for this example $\displaystyle \vec{A}=\begin{pmatrix} 2 & 0 \\ 1 &  1 \end{pmatrix}$.
...
```{r,fig.cap="phase plane for @eq-example-ch15.",echo=FALSE}
#| label: fig-ex-ch15fig
...
Notice how in @fig-ex-ch15fig phaseline the arrows seem to spin out from the origin? We are going to discuss that below - but based on what we see we might expect the stability of the origin to be _unstable_. 

## Non-equilibrium solutions and their stability 
Even though we have already identified the equilibrium solution for @eq-example-ch15, there are other non-equilibrium solutions. Here are two non-equilibrium solutions that you can verify:
...
These two non-equilibrium solutions can aid us in understanding the stability of the equilibrium solutions. All of the solutions contain terms that are exponential growth, indicating movement away from the equilibrium solution at $x=0$, $y=0$. This is even more evident when we plot the solutions with the phase plane by defining a new data frame and plotting $s_{1}(t)$ and $s_{2}(t)$ with `geom_path` (@fig-ex-ch15fig-sol):


```{r,fig.cap="phase plane for @eq-example-ch15, with straight line solutions $s_{1}(t)$ and $s_{2}(t)$.",echo=FALSE,warning = FALSE, message = FALSE}
#| label: fig-ex-ch15fig-sol
...
phaseplane(systems_eq, "x", "y") +
  geom_path(data = solution, aes(x = x, y = y, color = as.factor(soln)), linewidth = 2) +
...
The arrows in the phase plane of @fig-ex-ch15fig-sol show how $s_{1}(t)$ and $s_{2}(t)$ move away from the solution. The phase plane suggests for @eq-example-ch15 that the equilibrium solution at the origin is unstable because _both_ the arrows in both directions seem to be pointing away from the origin. 

Also notice that in @fig-ex-ch15fig-sol the solutions $s_{1}(t)$ and $s_{2}(t)$ in the $xy$ plane are straight lines!  It turns out that these straight line solutions are quite useful - we will study them in a later chapter.
...
The vectors $\displaystyle  \begin{pmatrix} 0 \\ 1\end{pmatrix}$ and $\displaystyle  \begin{pmatrix} 1 \\ 1\end{pmatrix}$ are the lines $x=0$ and $y=x$, as shown in @fig-ex-ch15fig-sol. By factoring out the exponential functions we can see the straight line solutions!  
...
```{r,fig.cap="Phase plane for @eq-example-ch15, with straight line solutions $s_{1}(t)$, $s_{2}(t)$, and $s_{3}(t)$.",warning = FALSE, message = FALSE,echo=FALSE}
#| label: fig-ex-ch15fig-sol-2
# Define the straight line solutions:
solution1 <- tibble(
  t = seq(0, 2, length.out = 20),
  x = 0,
...
::: {#exr-soln-verify-15}
Verify that $\displaystyle s_{1}(t) =  \begin{pmatrix} 0 \\ e^{t} \end{pmatrix}$ and  $\displaystyle s_{2}(t) = \begin{pmatrix} e^{2t} \\ e^{2t} \end{pmatrix}$ are both solutions for @eq-example-ch15.
:::


::: {#exr-soln-verify2-15}
Verify that $\displaystyle s_{3}(t) = \begin{pmatrix} x \\ y \end{pmatrix} = \begin{pmatrix} -0.1e^{2t} \\ e^{t} - 0.1e^{2t} \end{pmatrix}$ is a solution for @eq-example-ch15.
:::


::: {#exr-soln-lin-comb-15}
Generate a new solution for @eq-example-ch15 that is a linear combination of $s_{1}(t)$ and $s_{2}(t)$ and plot your solution with the phase plane for @eq-example-ch15.
:::
...
::: {#exr-phase-matrix-15}
Generate a phase plane for each of the systems in @exr-matrix-15 and classify the stability of the equilibrium solution at $x=0$ and $y=0$ as stable, unstable, or uncertain, providing reasons for your conclusion.
:::
