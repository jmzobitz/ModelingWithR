# Bifurcation {#sec-bifurcation-20}
...
Consider the differential equation $\displaystyle x' = 1-x^{2}$. This differential equation has an equilibrium solution at $x=\pm 1$. To classify the stability of the equilibrium solutions we apply the following test for stability of an equilibrium solution that we developed in @sec-phase-05:
...
```{r, echo=FALSE,results='hide',warning=FALSE,fig.cap="Phase plane with associated solutions for $\\displaystyle x'=c-x^{2}$ for different values of $c$. The dashed grey lines are equilibrium solutions.",fig.width=4,fig.height=12,fig.pos="!p"}
#| label: fig-eq1-ex2-3
...
@fig-eq1-ex2-3 builds on an interesting pattern in our series of differential equations. Let's build on these three examples in a more general context. Consider the differential equation $\displaystyle x' = c-x^{2}$, which is a generalization of our examples. (For our examples $c=1$ $-1$, and $0$ respectively.)
...
Even though in this case the stability test is inconclusive, based on the phase plane for $x'=-x^{2}$ (@fig-eq1-ex2-3), the equilibrium solution $x^{*}=0$ is unstable. If the initial condition $x(0)$ is greater than 0, the solution flows towards $x=0$, but if the initial condition is less than zero, the solution flows away from $x=0$. This type of behavior is similar to a one-dimensional analogue to a saddle node from @sec-eigenvalues-18.
...
```{tikz, label=fig-saddle-bifur-20,warning=FALSE,message=FALSE,echo=FALSE,fig.cap="A saddle node bifurcation for the differential equation $\\displaystyle x'=c-x^{2}$.",fig.align='center'}
...
Let's talk about @fig-saddle-bifur-20. The graph represents the value of the equilibrium solution ($x^{*}$, vertical axis) as a function of the parameter $c$ (horizontal axis). Since equilibrium solutions are characterized by $x^{*}=\pm \sqrt{c}$ we have the "sideways parabola", traced in blue in @fig-saddle-bifur-20. When $c<0$, there is no equilibrium solution, (so nothing is plotted in the second and third quadrants of @fig-saddle-bifur-20). The difference between the solid and dashed lines in @fig-saddle-bifur-20 is used to distinguish between a stable equilibrium solution ($x^{*}=\sqrt{c}$ when $c>0$) and an unstable equilibrium solution ($x^{*}=-\sqrt{c}$ when $c>0$). It is so cool that *all* the information about the equilibrium solution and its stability is contained in @fig-saddle-bifur-20!
...
```{r, echo=FALSE,results='hide',warning=FALSE,fig.cap="Phase plane for @eq-b-bifurc-20 when $b=-2$"}
#| label: fig-bifuc-b-20
...
```{r, echo=FALSE,results='hide',warning=FALSE,fig.cap="Phase plane for @eq-b-bifurc-20 when $b=0$"}
#| label: fig-bifuc-b-20-2
...
```{r, echo=FALSE,results='hide',warning=FALSE,fig.cap="Phase planes for @eq-b-bifurc-20) for different values of $b$.",fig.width=4,fig.height=12}
#| label: fig-bifuc-b-20-3
...
This equation has one free parameter $b$ that we will analyze using the trace determinant conditions developed in @sec-stability-19. Let's call the matrix $A$, so the tr$(A)=4$ and $\det(A)=3-b$. Since the trace is always positive either the equilibrium solution will be a saddle if $\det(A)<0$, or when $3<b$. We have a spiral source when $\det(A)>0$ (this means $3 > b$) and $\det(A) > (\mbox{tr}(A))^{2}/4$, or when $3-b > 4$, which leads to $b<-1$. Figures [-@fig-bifuc-b-20-3]A-C displays the phase planes for different values of $b$ along with sample solution curves.

To summarize, @eq-b-bifurc-20 has the following dynamics depending on the value of $b$:
...
Another approach to analyzing @eq-b-bifurc-20 is to compute the eigenvalues directly, which in this case are $\displaystyle \lambda_{1,2}(b)=2 \pm \sqrt{b+1}$. Creating a plot of the eigenvalues (@fig-bifurc-b-lambda) can also help explain the bifurcation structure. When $b<-1$, the eigenvalues are imaginary, with Re$(\lambda_{1,2}(b)=2)=2$, so the equilibrium solution is a spiral source. When $-1 < b < 3$, both eigenvalues are positive, so the equilibrium solution is a source. Finally, when $3 < b$, one eigenvalue is positive and the other is negative, confirming our analyses with the trace-determinant plane.

```{r,echo=FALSE,results='hide',warning=FALSE,fig.cap="Bifurcation diagram for @eq-b-bifurc-20. The vertical axis shows the value of the eigenvalues $\\lambda$ (red and blue curves) as a function of the parameter $b$. The annotations represent the stability of the original equilibrium solution." }
#| label: fig-bifurc-b-lambda
...
p1 <- in_data |>
  ggplot() + 
  geom_line(aes(x=b,y=y1),color='red',linewidth=1) +
  geom_line(aes(x=b,y=y2),color='blue',linewidth=1) +
...
The phase plane for @eq-limit-cycle-20 is shown in @fig-limit-cycle-20-f. You can verify that @eq-limit-cycle-20 has an equilibrium solution at the point  $x=0$, $y=0$. However @fig-limit-cycle-20-f suggests there might be other equilibrium solutions when various solution curves are plotted with the phase plane.

```{r,warning=FALSE,echo=FALSE,fig.width=3,fig.height=3,fig.cap="Phase plane for @eq-limit-cycle-20 with different solution curves. Notice the equilibrium solution described by the equation $x^{2}+y^{2}=1$."}
#| label: fig-limit-cycle-20-f
mu <- 1
...
What is interesting in @fig-limit-cycle-20-f is that the solution tends towards a circle of radius 1 (or the equation $x^{2}+y^{2}=1$). This is an example of an equilibrium solution that is a *curve* rather than a specific point. We can describe the phase plane with a new variable $X$ that represents the distance from the origin (a radius $r$) by transforming this system from $x$ and $y$ to a single new variable $X$ (see @exr-limit-transform-20).

$$
\frac{dX}{dt} = -X(X-1) \mbox{ where } X=r^{2}
$$  {#eq-limit-X-20}

How @eq-limit-cycle-20 transforms to @eq-limit-X-20 is by applying a polar coordinate transformation to this system. With stability analysis for @eq-limit-X-20 we can show that the equilibrium solution $X=0$ is unstable (meaning the origin $x=0$ and $y=0$ is an unstable equilibrium solution) and the circle of radius 1 is a stable equilibrium solution (which is the equation $x^{2}+y^{2}=1$). In this case we would say $r=1$ is a *stable limit cycle*.\index{limit cycle}\index{limit cycle!stable}  You will study a similar system in [Exercises @exr-limit-transform-20 and @exr-hopf-20].

@eq-limit-X-20 is an example of next steps with studying the qualitative analysis of systems. We can extend out @eq-limit-cycle-20 further to introduce a parameter $\mu$ that, as $\mu$ changes, undergoes a bifurcation as $\mu$ increases. This is an example of a *Hopf bifurcation*.\index{bifurcation!Hopf} 
...
e. Create a plot similar to @fig-bifurc-b-lambda showing the bifurcation structure.
