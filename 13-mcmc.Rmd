...
We will work you through this step by step, with example code that you can type along the way. As always it will be good to load up the libraries you will be using.  This section will examine two examples: one with a differential equation and one without.
...
SHOULD WE TALK ABOUT TIMING THE CODE TO GET A SENSE OF SCOPE?
### Timing your code
As you can imagine the more iterations we have the better our parameter estimates will be.  This comes with a tradeoff: it takes some time to run the full estimate.  To get a sense of timing the code there is a helpful clock function in `R` that starts a stopwatch if you will.  Do a smaller number of iterations will then give you a sense of the proportional scaling.
