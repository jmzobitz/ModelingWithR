...
### Alternative approach with R functions {#random-sample-subsec}
...
```{rw2d-23,fig.cap="Simulation of 100 steps of a two dimensional random walk.",echo=FALSE,message=FALSE,warning=FALSE}
# Number of steps our random walk takes
number_steps <- 100
...
Figure \@ref(fig:rw2d-23) shows how the position evolves over time (using the color gradient).

## Iteration and ensemble averages {#iteration-sample-subsec}
...
## Exercises
```{exercise}
Rerun the code from @random-sample-subsec that implements the random walk that uses the function `sample` with the option `replace = FALSE`. You should receive an error. Explain the error in your own words.
```

```{exercise}
Rewrite the function `random_walk` defined in @iteration-sample-subsec using the `sample` function defined in @random-sample-subsec.  You may have to adjust the vector `t` when defining the output tibble.
```

```{exercise cumsum-test}
