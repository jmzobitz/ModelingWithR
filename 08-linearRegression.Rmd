...
# Linear Regression & Curve Fitting {#linear-regression-08}
...
(This dataset can be found in the `MAT369Code` package with the name `global_temperature`.) To name our variables let $Y=\mbox{ Year since 1880 }$ and $T= \mbox{ Temperature }$. First let's visualize the data, with time on the horizontal axis and temperature on the vertical axis:
...
ggplot(data = global_temperature) +
  geom_point(aes(x=yearSince1880,y=globalTemp),
             color='red',
             size=2) +
  geom_line(data = global_temperature_model,
            aes(x=yearSince1880,y=.fitted)) +
  labs(x='Year Since 1880',
