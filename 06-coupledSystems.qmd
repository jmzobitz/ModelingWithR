# Coupled Systems of Equations {#sec-coupled-06}
@sec-phase-05 focused on qualitative analysis of a single differential equation using phase lines and slope fields. This chapter extends this idea further to systems of differential equations, where the natural extension of a phase line is a *phase plane*.\index{phase plane} Here is the good news: many of the techniques are similar to the ones introduced in @sec-phase-05. Let's get started!

::: {.callout-note}
This chapter uses following libraries: `ggplot2` (part of the `tidyverse`) and `demodelr`. See @sec-packages-02 on how to install packages.  Prior to running any code in this section, include `library(tidyverse)` and `library(demodelr)`. 
:::

## Flu with quarantine and equilibrium solutions

In  @exr-flu-quarantine-01 in @sec-intro-01 we developed the following model for the flu as a coupled system of equations shown in @eq-flu-quarantine-06:
...
\frac{dR}{dt} &= \beta I,
\end{split}
$$ {#eq-flu-quarantine-06}

where $S$ represents susceptible people, $I$ infected people, and $R$ recovered people. Another way to represent this context is with the schematic shown in @fig-quarantine:


```{tikz,label=fig-quarantine,warning=FALSE,message=FALSE,echo=FALSE,fig.align="center",fig.cap="Schematic of the flu model with quarantine."}
...
While @eq-flu-quarantine-06 is a system of three differential equations, notice that the variable $R$ is not present on the right hand sides of each equation. As a result, the variable $R$ is decoupled from this system of equations, so we can just focus on the rates of change for $S$ and $I$ (@eq-flu-quarantine-small-06):
...
Nullclines are not equilibrium solutions by themselves - it is the _intersection_ of two different nullclines that determines equilibrium solutions. @fig-quarantine-nullclines shows the nullclines in the $S-I$ plane (since we have two equations), with $S$ on the horizontal axis and $I$ on the vertical axis. In @fig-quarantine-nullclines we have also assumed that $\beta=1$ and $k=1$. The $S-I$ plane shown in @fig-quarantine-nullclines is the beginning of the construction of the phase plane for @eq-flu-quarantine-small-06 and also to determine the equilibrium solutions.


```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Nullclines for @eq-flu-quarantine-small-06. To generate the plot we assumed $\\beta=1$ and $k=1$."}
#| label: fig-quarantine-nullclines
k <- 1
beta <- 1
ggplot(data.frame(x = c(0, 2)), aes(x)) +
  ...
  ggthemes::scale_color_colorblind(name = "Nullclines:", breaks = c("S' = 0", "I' = 0"))
```

A key thing to note is that where two different nullclines cross is an *equilibrium solution* to the system of equations (**both** $\displaystyle \frac{dS}{dt}$ and $\displaystyle \frac{dI}{dt}$ are zero at this point). Examining @fig-quarantine-nullclines three possibilities appear:
...
3. There is also a third possibility where the vertical line at $S=1$ crosses the horizontal axis ($S=1$, $I=0$), but that also falls under the second equilibrium solution.^[For the general system (@eq-flu-quarantine-small-06), the equilibrium solution would that be $\displaystyle S=\frac{\beta}{k}$ and $I=0$.]
...
## Phase planes
Next we can add more context to the @fig-quarantine-nullclines by evaluating different values of $S$ and $I$ into our system of equations and plotting the _phase plane_.\index{phase plane} How we plot the phase plane is similar to the method in @sec-phase-05. We will test points around an equilibrium solution to determine if the solution is increasing or decreasing in $S$ or $I$ independently.

@tbl-slope-arrows-06 evaluates the derivatives $\displaystyle \frac{dS}{dt}$ and $\displaystyle \frac{dI}{dt}$ in @eq-flu-quarantine-small-06 for different values of $S$ and $I$.


```{r echo=FALSE,message=FALSE,warning=FALSE}
#| label: tbl-slope-arrows-06
k <- 1
beta <- 1
...
model_out <- expand.grid(S, I) |>
  ...
  round(digits = 2)
kable(t(model_out), caption = "Values of $\\displaystyle \\frac{dS}{dt}$ (as `dSdt`) and $\\displaystyle \\frac{dI}{dt}$ (as `dIdt`) for @eq-flu-quarantine-small-06.")
```

Notice in @tbl-slope-arrows-06 the different values of $\displaystyle \frac{dS}{dt}$ and $\displaystyle \frac{dI}{dt}$ in @eq-flu-quarantine-small-06 at each of the given $S$ and $I$ values. We can plot each of the coordinate pairs of $\displaystyle \left( \frac{dS}{dt}, \frac{dI}{dt} \right)$ as a vector (arrows) in the $(S,I)$ plane. To do so, associate $\displaystyle \frac{dS}{dt}$ with left-right motion, so positive values of $\displaystyle \frac{dS}{dt}$ mean the vector points to the right. Likewise, we associate  $\displaystyle \frac{dI}{dt}$ with up-down motion, so positive values $\displaystyle \frac{dI}{dt}$ mean the vector points up.

Defining the directions of the vectors in this way is also consistent when @eq-flu-quarantine-small-06 is evaluated at the nullcline solutions. At the point $(S,I)=(1,1)$, we have an arrow that points directly to the west because $\displaystyle \frac{dS}{dt} < 0$ and $\displaystyle \frac{dI}{dt} =0$. Continuing on in this manner, by sequentially sampling points in the $(S,I)$ plane we get a vector field plot (@fig-slope-field-06), superimposed with the nullclines.

```{r, echo=FALSE, message = FALSE, warning = FALSE, fig.cap="Phase plane for @eq-flu-quarantine-small-06, with $\\beta=1$ and $k=1$."}
#| label: fig-slope-field-06
k <- 1
beta <- 1
s_window <- c(0, 2)
i_window <- c(0, 2)
...
Applying this knowledge to @eq-flu-quarantine-small-06, if we choose points where $I'=0$ then we know that the only motion is to the left and the right because $S$ can still change along that curve. If we choose points where $S'=0$ then we know that the only motion is to the up/down because $I$ can still change along that curve.
...
The `demodelr` package has some basic functionality to generate a phase plane. Consider the following system of differential equations (@eq-phase-example):
...
In order to generate a phase plane diagram for @eq-phase-example we need to define functions for $x'$ and $y'$, which I will annotate as $dx$ and $dy$ respectively. We are going to collect these equations in one vector called `system_eq`, using the tilde (~) as a replacement for the equals sign:
...
```{r,echo=FALSE,fig.cap="Phase plane for @eq-phase-example."}
phaseplane(system_eq,
  x_var = "x",
  y_var = "y"
) +
...
- Next you need to define which variable belongs on the horizontal axis (`x_var = 'x'`) or the vertical axis (`y_var = 'y'`). In  @exr-wacky-phase you will explore what happens if these get mixed up.
...
$$ {#eq-bacteria-colony-06}


:::

The `R` code shown below will generate the slope field for @eq-bacteria-colony-06 (shown in @fig-phaseline-06):
...
```{r, echo=FALSE,fig.cap="Slope field for @eq-bacteria-colony-06."}
#| label: fig-phaseline-06
# Define the windows where we make the plots
t_window <- c(0, 3)
x_window <- c(0, 5)
...
A few notes about the code that generated @fig-phaseline-06:
...
- Notice how the variable `system_eq` also contains the additional equation `dt = 1`. What we are doing is re-writing @eq-bacteria-colony-06 by introducing a new variable $s$ (@eq-single-phase):
...
The differential equation $\displaystyle \frac{dt}{ds} = 1$ has a solution $s=t$, so really @eq-single-phase is a (slightly more) complicated way to express @eq-bacteria-colony-06. Hacky? Perhaps. However re-writing @eq-bacteria-colony-06 was a quick and handy workaround to re-use code.

This chapter introduced a lot of useful `R` code to aid in visualization. The good news is that we will explore additional analyses of systems of differential equations starting with @sec-linearsystems-15 - there is so much more to learn. Onward!

## Exercises

:::{#exr-phase-eq}
Determine equilibrium solutions for @eq-phase-example.
:::

:::{#exr-phase-eq-num}
Generate a phase plane for @eq-phase-example, but set the option `eq_soln = TRUE`. Did `phaseplane` detect the equilibrium solution you found in  @exr-phase-eq? If not, repeat the phase plane, but set `x_window = c(-0.1,0.1)` and `y_window = c(-0.1,0.1)` and repeat.
:::

:::{#exr-wacky-phase}
Generate a phase plane for @eq-phase-example, but this time set `x_var = y` and `y_var = x` (swap which variable is which). Notice that the incorrect phase plane is produced. What is the corresponding differential equation that is visualized by this phase plane?
:::
...
::: {#exr-lynx-06}
Consider the following system of differential equations for the lynx-hare model (@eq-lynx-hare-combined from @sec-modeling-rates-03):
...
a.  Determine the equations of the nullclines and equilibrium solutions for this system of differential equations.
b.  For each point along a nullcline, determine the resulting motion (up-down or left-right).
c.  Modify the function `phaseplane` to generate a phase plane of this system. Set $N=100$.
d.  Make a hypothesis to classify if the equilibrium point is *stable* or *unstable*.
