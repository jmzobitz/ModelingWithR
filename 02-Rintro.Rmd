...
Another data frame may list solutions to a differential equation, like we did with our three infection models in Chapter \@ref(intro-01) (Table \@ref(tab:model-table)).
...
a. If you haven't already, make a scatterplot (\texttt{geom\_point()}) of the dataset \texttt{phosphorous} in the package library. Be sure to label the axes correctly.
b. Use your function \texttt{nutrient} from the previous exercise to make an initial guess for \texttt{theta} ($\theta$) that would be consistent with the data.  You can evaluate your guess by plotting (with \texttt{geom\_line()}) against the data.
...
a. Explain (using your own words) what the function \texttt{runif(1,100,1000)} does.
b. Explain (using your own words) what the function \texttt{ceiling()} does, showing an example of its use.
...
a. Create a new source file and save it as \texttt{myFunction.R}.
b. Type this code the file you created: \texttt{myInteger <- ceiling(runif(1, 100, 1000))} (This will declare a variable \texttt{myInteger} that you will work with in the following steps.)
c. Determine a function in R that will \textbf{compute a cumulative sum} from 1 to the value of \texttt{myInteger}.  Modify your file so that it also computes the cumulative sum and then source your file.
...
a. Define a function in \texttt{R} that computes $y$ for $0 \leq x \leq 2$.
b. Use \texttt{geom\_line()} to generate a graph of $y(x)$ over the interval $0 \leq x \leq 2$.
...
a. Define a function in \texttt{R} that computes $r$ for $0 \leq T \leq 30$.
b. Use \texttt{geom\_line()} to generate a graph of $r(T)$ over the interval $0 \leq T \leq 30$.
