...
```{r r-studio-pane, echo=FALSE,out.width = "5in",fig.align="center",fig.cap="A sample RStudio pane from one of my projects."}
...
```{r, echo=FALSE,out.width = "5in",fig.align="center",fig.cap="Sending a command to the console."}
...
- Github.  This is another place where people can share code and packages.  The code here has not been vetted through CRAN for compatibility, but if you trust the person sharing the code, it should work.  In order to do this we will need to first load up the `devtools` library:

```{r eval=FALSE}
library(devtools)
install_github("jmzobitz/demodelr",build_vignettes= TRUE)
```
...
```{r, echo=FALSE,out.width = "5in",fig.align="center",fig.cap="A sample R script."}
...
Here is the good news, then we can compute with the new variable, so for example `10*my_result` should yield 130.  Cool, no?  As an example, let's define a sequence, spaced from 0 to 5 with spacing of 0.05.  Store this in a variable called `my_sequence`.
...
- `ggplot(data = wilson) + ` sets up the graphics structure and identifies the name of the data frame we are include.  
