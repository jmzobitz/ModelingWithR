...
```{r non-lin-phase-18,echo=FALSE,fig.width=4,fig.height=3,fig.cap="A representative phase plane."}
...
phaseplane(system_eq, "x", "y", x_window = c(-2, 2), y_window = c(-2, 2)) +
  geom_point(data=tibble(xint=c(-1,1),yint=c(1,1)),aes(x=xint,y=yint),color='red',size=4)
```


Are the equilibrium solutions (red dots in Figure \@ref(fig:non-lin-phase-18)) stable?  In this section we will develop an algebraic process to analyze locally linear behavior, connecting what we learned to straight line solutions (Section \@ref(linearsystems-15)) and the Jacobian (Section \@ref(jacobian-17)).
...
```{example}
Compute the eigenvectors for the matrix $\displaystyle A =  \begin{pmatrix} -1 & 1 \\ 0 &  3 \end{pmatrix}$.
```

```{solution}
So if we consider the right hand side of $A \vec{v} - \lambda \vec{v} =\vec{0}$ we have:
...
```{solution}
Since we have already computed the eigenvalues and eigenvectors, our most general solution is:
...
### Eigenvalues computed with demodelr
...
Plotting both solutions versus time shows woudl show $x$ and $y$ increase in time.
...
Notice the trigonometric terms in Equation \@ref(eq:imag-eval-18) - we should expect the solution to be periodic and to have some periodic behavior to it.  In fact, When $\alpha < 0$ we say the equilibrium solution is a _spiral sink_ because the exponential terms in the solution decay asymptotically to zero.  
...
```{exercise}
(Inspired by @logan_mathematical_2009) Compute the eigenvalues and eigenvectors for the following linear systems.  Based on the eigenvalues, classify if the equilibrium solution is stable or unstable.  Finally write down the most general solution for the system of equations.


a. $\displaystyle \frac{dx}{dt} = -x, \;  \frac{dy}{dt} = -y$
b. $\displaystyle \frac{dx}{dt} = 3x-2y, \;  \frac{dy}{dt} = 2x-2y$
c. $\displaystyle \frac{dx}{dt} = -4x+2y, \;  \frac{dy}{dt} = x-3y$
d. $\displaystyle \frac{dx}{dt}= 4y, \; \frac{dy}{dt}=-9x$
e. $\displaystyle \frac{dx}{dt} = y, \;  \frac{dy}{dt} = -x$
...
c. Now use the previous results to compare $\displaystyle \frac{d}{dt} \vec{s}(t) = A \vec{s}(t)$. Explain why it must be the case that $\lambda \vec{v} = A \vec{v}$
