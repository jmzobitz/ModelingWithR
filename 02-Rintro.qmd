...
::: {.callout-note}
This chapter uses following libraries: `ggplot2`, and `tibble` (all part of the `tidyverse`) and `demodelr`. See Section \@ref(packages-02) on how to install packages.  Prior to running any code in this section, include `library(tidyverse)` and `library(demodelr)`. 
:::
...
## Increasing functionality with packages {#packages-02}
