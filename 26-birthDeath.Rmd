...
In the last section we examined how we built up a stochastic differential equation from adding in stochasticity (noise) to the parameter values.  Another approach is to assume the variables are stochastic.  We will be able to simulate stochastic equations, setting them up in a similar way (deterministic and stochastic parts) to generate realizations and then compute the ensemble averages.
...
```{r random-walk-stoch,engine='tikz',warning=FALSE,message=FALSE,echo=FALSE}
\begin{center}
\begin{tikzpicture}
    \draw (-3,0) -- (3,0);
...
Along with the computation of the variance,  We are going to compute the variance to first order in $\Delta t$.  Because of that, we are going to assume that in $\sigma^{2}$ any terms involving $(\Delta t)^{2}$ are small, or in effect neglibile.  This is a huge simplifying assumption for the variance, but it is useful!
...
Notice how the last step provides a way to generate a solution trajectory to our differential equation.  Cool!  Let's implement this in `R`.  I have code that can simulate this process once you identify the key parameters and elements of the code.  I call this a birth death process.
...
In this way we think of $\alpha(x)$ as a "birth process" and $\delta(x)$ as a "death process". When we computed the mean $\mu$ and variance $\sigma^{2}$ we had $\mu = \alpha(x)-\delta(x)$ and $\sigma^{2}=\alpha(x)+\delta(x)$.  This relationship holds up for *any* differential equation where we have identified a birth or death process.


It turns out that this particular splitting is not unique, and we can obtain different dynamics depending on the split.  Let's take a look at an example
...
## Exercises
Do a birth death process with the birth term the entire equation, death process zero?
Identify alpha delta for a series of equations (just write them down?)
