...
### Pivot
When you have multiple columns of a plot that you want to show together (such as a spaghetti plot) we can use a command from the `tidyr` package called [`pivot_longer`](https://tidyr.tidyverse.org/articles/pivot.html) that gathers different columns together (also shown in Table \@ref(tab:simul-table-long)).
