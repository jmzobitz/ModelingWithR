...
```{r,fig.cap="Spaghetti plot of the three simulations from @tbl-simul-table.",echo=FALSE}
#| label: fig-simul-graph-prev
...
While making the spaghetti plot isn't bad when you have three simulations, with (a lot) more simulations this would be a pain! An ensemble average computes *across* the rows in @tbl-simul-table (could be an average or a quantile) to generate a new column in the data. Building an ensemble average is a step by step process that involves a series of processes that transform a dataset where you will need to first pivot the dataset and then group and summarize.

### Pivot
When you have multiple columns of a plot that you want to show together (such as a spaghetti plot) we can use a command from the `tidyr` package called [`pivot_longer`](https://tidyr.tidyverse.org/articles/pivot.html) that gathers different columns together (also shown in @tbl-simul-table-long).
...
We can add this mean value to our data (@fig-simul-graph-prev), represented with a thick red line. Try the following code out on your own:
...
### Do once
To investigate the effect of the initial condition on the solution we will choose the initial condition from a uniform distribution between 0 and 20, shown in the following code and plotted in @fig-logistic-example-rev:
...
```{r,fig.cap="Two solutions to the logistic differential equation with a random initial condition.",echo=FALSE}
#| label: fig-logistic-example-rev
...
```{r,fig.cap="Spaghetti plot for logistic differential equation with 500 random initial conditions.",echo=FALSE}
#| label: fig-logistic-example-many
...
-   `map(1:n_sims, ~ c(x = runif(1, min = 0, max = 20)))` This line does two things: `x=runif(1,min=0,max=20)` makes a random initial condition, and the command `map` runs this again for `n_sims` times.
...
matplot(logistic_sim_wide[,1], logistic_sim_wide[, -1], 
        type = "l", lwd = 2, lty = 1,
        xlab = "t", ylab = "x",main="Random initial conditions")
```
...
- We use [`reframe`](https://dplyr.tidyverse.org/reference/reframe.html) instead of `summarize` because we used the `quantile` function that produces several outputs (instead of a single value).
...
- The command `pivot_wider` from `tidyr` takes the values in `q_val` with the associated names in `q_name` and creates new columns associated with each quantile. This process of making a data frame wider is the opposite of making a skinny and tall data frame with `pivot_longer`. A key convention with column names is not to start them with a number, so we *glue* a `q` onto the names of the column using the option `names_glue = "q{q_name}"`.
...
# Update column names
colnames(log_quantile_apply) <- paste0("q", colnames(log_quantile_apply))
...
```{r,fig.cap="Ensemble average plot for the logistic differential equation with 500 random initial conditions.",echo=FALSE}
#| label: fig-logistic-conf
logistic_quantile |>
  ggplot() +
  geom_line(aes(x = t, y = q0.5), 
...
::: {#exr-adjust-fig-22}
Using the code to produce @fig-logistic-conf:
...
d. Compare your results along with Figures [-@fig-logistic-example-many] and [@fig-logistic-conf]. How does randomizing the initial condition or the parameters affect the results?
