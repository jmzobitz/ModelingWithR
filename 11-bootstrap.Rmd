...
- The code `aes(x = snowfall)` is computing the histogram for the `snowfall` column in the dataset `snowfall`. You may have received a warning about the bins ``stat_bin()` using `bins = 30`. Pick better value with `binwidth``, so let's adjust the number of bins to `r round(sqrt(length(MAT369Code::snowfall$snowfall)),digits=0)`:
