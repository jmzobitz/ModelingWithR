...
```{r moose-det-21,fig.width=4,fig.height=3,fig.align='center',fig.cap="Simulation of the moose population with different birth rates."}
...
Ok, so we found our equilibrium solution.  However what is the general form of this solution to Equation \@ref(eq:moose)? Just like when we were solving continuous differential equations (Section \@ref(exact-solns-07) a starting point was that the solution was exponential. For discrete systems we will do the same, but this time we represent the solution as $M_{t}=A\cdot v^{t}$, which is an exponential equation.  Since we have $M_{0}=3000$, then $A=3000$.  Using these values of $M_{0}$ and $A$ for Equation \@ref(eq:moose) we obtain the following:
...
M_{t+1} &= G_{J} \cdot J_{t} + P_{M} \cdot M_{t}
...
b. Explain why the equation that describes the dynamics is $$ u_{t+1}=ru_{t} - \epsilon \left( \frac{w \cdot s}{a} u_{t} \right) $$
