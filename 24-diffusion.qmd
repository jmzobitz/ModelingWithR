...
{{< fa burst >}} Notice the use of the [`map`](https://purrr.tidyverse.org/reference/map.html) function from the `purrr` library. This is a function to do repeated iteration. If you prefer base `R` implementation, you can replace `map(1:n_sims, ~ list())` with `replicate(n_sims, { list() })` or even `rep(list(), length.out = n_sims)`.
...
::: {#exr-brownian-alt-24}
{{< fa burst >}} The function `brownian_motion` can also be implemented using the function `cumsum`, or cumulative summation:
...
Evaluate `brownian_motion_alt` to recreate @fig-sample-brownian.
:::
