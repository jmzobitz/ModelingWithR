...
In @sec-linearsystems-15 we identified two straight line solutions:
...
While we wrote the right hand side of @eq-system-ch18 component by component, we could also write it as $A \vec{x}$, where $\displaystyle A = \begin{pmatrix} 2 & -1 \\ 2 &  5 \end{pmatrix}$. Because we verified $\vec{s}_{2}(t)$ was a solution to the differential equation, we could also have said that $A \vec{s}_{2}(t) = 3 \vec{s}_{2}(t)$.
...
The determinant of a 2 $\times$ 2 matrix is formed by the product of the diagonal entries less the product of the off-diagonal entries. For @eq-2by2-ch18, $\det(A-\lambda I)=0$ is the equation $(a-\lambda)(d-\lambda)-bc=0$. Our goal is to solve this equation for $\lambda$, which are the eigenvalues for this system. 
...
::: {#exm-evec-18-2}
Compute the eigenvectors for the matrix $\displaystyle A =  \begin{pmatrix} -1 & 1 \\ 0 &  3 \end{pmatrix}$ from @exm-evec-18.
:::
...
Once we have computed the eigenvalues and eigenvectors, we are now ready to express the most general solution for a system of differential equations. For a two-dimensional system of linear differential equations ($\displaystyle \frac{d}{dt} \vec{x} = A \vec{x}$), the most general solution is given by @eq-gen-soln-18:
...
Consider the differential equation in @eq-source-ex-18:

$$ 
\begin{pmatrix} x' \\ y' \end{pmatrix} =\begin{pmatrix} 2 & 0 \\ 1 &  1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
$$ {#eq-source-ex-18}

Computing the eigenvalues for @eq-source-ex-18 shows that they are both positive:
```{r}
eigenvalues(c(2, 0, 1, 1))
```

The phase plane for this matrix $A$ is shown in @fig-source-18:

```{r, echo=FALSE, fig.cap="Phase plane for @eq-source-ex-18, which shows the equilibrium solution is a source (also known as an unstable node)."}
#| label: fig-source-18
...
Notice how the phase plane in @fig-source-18 has all the arrows pointing from the origin. In this case we call the origin equilibrium solution a _source_, or also an _unstable node_.\index{equilibrium solution!source}\index{equilibrium solution!unstable node} Plotting the components of $\vec{x}(t)$ as functions of $t$ would show the dependent values increase exponentially as time increases.



### Sinks: all eigenvalues negative
Consider the differential equation in @eq-sink-ex-18, which is a _slight_ modification from @eq-source-ex-18:

$$
\begin{pmatrix} x' \\ y' \end{pmatrix} =\begin{pmatrix} -2 & 0 \\ 1 &  -1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
$$ {#eq-sink-ex-18}

The eigenvalues for @eq-sink-ex-18 are both negative (verify this on your own). The resulting phase plane for @eq-sink-ex-18 then has all the arrows pointing towards the origin, shown in @fig-sink-18.

```{r,echo=FALSE, fig.cap="Phase plane for @eq-sink-ex-18, which shows the equilibrium solution is a sink (also known as a stable node)."}
#| label: fig-sink-18
# For a two-variable system of differential equations we need to define dx/dt and dy/dt separately:
...
Based on the phase plane in @fig-sink-18, solutions to @eq-sink-ex-18 would asymptotically approach the origin. We say the equilibrium solution is a  _sink_, also known as a _stable node_.\index{equilibrium solution!sink}\index{equilibrium solution!stable node} 


### Saddle nodes: one positive and one negative eigenvalue
Consider the differential equation in @eq-saddle-ex-18:

$$ 
\begin{pmatrix} x' \\ y' \end{pmatrix} =\begin{pmatrix} 3 & -2 \\ 1 &  -1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
$$ {#eq-saddle-ex-18}

@eq-saddle-ex-18 has $\lambda_{1}=2.414$ and $\lambda_{2}=-0.414$ (verify this on your own). Because the differential equation has one positive and one negative eigenvalue the equilibrium solution the phase plane for this differential equation looks a little different, as is shown in @fig-saddle-18:

```{r ,echo=FALSE, fig.cap="Phase plane for @eq-saddle-ex-18, which shows the equilibrium solution is a saddle node."}
#| label: fig-saddle-18
# For a two-variable system of differential equations we need to define dx/dt and dy/dt separately:
...
The phase plane in @fig-spiral-sink-18 has some spiraling motion to it. Why does that occur? Imaginary eigenvalues can occur when the characteristic equation $\det(A-\lambda I)=0$ has imaginary solutions.\index{eigenvalue!imaginary} More generally, we say $\lambda = \alpha \pm \beta i$. Because the eigenvalues are complex, we would also expect the eigenvectors to be complex as well (i.e. $\vec{v} \pm i \vec{w}$). Don't let the term _imaginary_ fool you: by using properties from complex analysis it can be shown that when eigenvalues are imaginary, the template for the solution is given in @eq-imag-eval-18:

$$
\vec{x}(t) = c_{1} e^{\alpha t} ( \vec{w} \cos (\beta t) - \vec{v} \sin (\beta t)) + c_{2} e^{\alpha t} ( \vec{w} \cos (\beta t) + \vec{v} \sin (\beta t)) 
$$ {#eq-imag-eval-18}

The trigonometric terms in @eq-imag-eval-18 suggest that the solution has some periodic behavior if we plot the components of $\vec{x}(t)$ as functions of $t$. But when we plot the solution in the $xy$ plane that periodic behavior gets translated to spiraling motion in @fig-spiral-sink-18. When $\alpha < 0$ we say the equilibrium solution is a _spiral sink_ because the exponential terms in @eq-imag-eval-18 decay asymptotically to zero.\index{equilibrium solution!spiral sink}


As you would expect when $\alpha > 0$ we classify a phase plane as a _spiral source_ (shown in @eq-spiral-source-ex-18 and @fig-spiral-source-18).\index{equilibrium solution!spiral source} 

$$
\begin{pmatrix} x' \\ y' \end{pmatrix} =\begin{pmatrix} 4 & -5 \\ 3 &  2 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
$$ {#eq-spiral-source-ex-18}


```{r,echo=FALSE, fig.cap="Phase plane @eq-spiral-source-ex-18 which shows the equilibrium solution is a spiral source."}
#| label: fig-spiral-source-18
# For a two-variable system of differential equations we need to define dx/dt and dy/dt separately:
...
The final case for imaginary eigenvalues is when $\alpha = 0$, which is termed a _center_.\index{equilibrium solution!center}  As an example, let's examine the phase plane for the system in @eq-center-ex-18:

$$ 
\begin{pmatrix} x' \\ y' \end{pmatrix} =\begin{pmatrix} 0 & -1 \\ 1 &  0 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
$$ {#eq-center-ex-18}

```{r,echo=FALSE, fig.cap="Phase plane for @eq-center-ex-18, which shows the equilibrium solution is a center."}
#| label: fig-center-18
# For a two-variable system of differential equations we need to define dx/dt and dy/dt separately:
...
Notice how the phase plane arrows in @fig-center-18 neither spin in nor out, but seem to point in a circle. In fact, the formula of the solution to this equation can be represented as a circle. The solution to this differential equation is a circle, which is found with $\alpha = 0$ in @eq-imag-eval-18.


### Repeated eigenvalues
For repeated eigenvalues (where $\lambda_{1}=\lambda_{2}$) the stability of the solution still depends on the sign of the sole eigenvalue $\lambda$.\index{eigenvalue!repeated} However in this case the form of the solution changes as shown in @eq-repeat-eval-18:

$$
\vec{x}(t) = \left( c_{1} \vec{v}_{1} + c_{2} \vec{v}_{2} \right) e^{\lambda t} + c_{2} \vec{v}_{1} t e^{\lambda t}
$$ {#eq-repeat-eval-18}
