# Information Criteria {#sec-information-criteria-14}
...
In [Exercises @exr-yeast-v1-13] and [-@exr-yeast-v2-13] of @sec-mcmc-13 we introduced two different empirical models for fitting the growth of `yeast` $V$ over time $t$. One model is a logistic model ($\displaystyle V = \frac{K}{1+e^{a-bt}}$), whereas the second model is a saturating function ($\displaystyle V= K + Ae^{-bt}$). A plot comparing MCMC parameter estimates for the two models is shown in @fig-yeast-compare-14.

```{r,fig.cap="Comparison of models for the growth of yeast in culture. Dots represent measured values from @gause_experimental_1932.",echo=FALSE}
#| label: fig-yeast-compare-14
...
@fig-yeast-compare-14 raises an interesting question. Sometimes we have multiple, convergent models to describe a context or situation. While having these different options is good, we also like to know which is the *best* model. How would you decide that?
...
Perhaps you may be asking, why bother? Aren't more models better? Let's talk about a specific example, for which we return to the dataset `global_temperature` in the `demodelr` library. Recall this dataset represents the average global temperature anomaly relative to 1951-1980. When we did linear regression with this dataset in @sec-linear-regression-08 the quadratic and cubic models were approximately the same (@fig-global-temp-14):

```{r,echo=FALSE,fig.cap="Comparison of global temperature anomaly dataset with various polynomial fitted models."}
#| label: fig-global-temp-14
...
Comparison of model fits for global temperature anomaly dataset shown in @fig-global-temp-14.
...
@tbl-gt-ic-14 compares $AIC$ and $BIC$ for the models fitted using the global temperature anomaly dataset:

<!-- Table: (\#tab:gt-ic-14) Comparison of the $AIC$ and $BIC$ for global temperature anomaly data shown in @fig-global-temp-14. -->
...
Comparison of the $AIC$ and $BIC$ for global temperature anomaly dataset shown in @fig-global-temp-14.

:::

@tbl-gt-ic-14 shows that the cubic model is the better approximating model for both the $AIC$ and the $BIC$.
