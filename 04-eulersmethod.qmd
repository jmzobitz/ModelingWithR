# Euler's Method {#sec-euler-04}
@sec-modeling-rates-03 examined modeling with rates of change. Once a differential equation model is defined one possible next step is to determine the solution to the differential equation. While in some cases an exact solution can be found (@sec-exact-solns-07), in many instances we will rely on numerical methods.
...
::: {.callout-note}
This chapter uses following libraries: `ggplot2`, and `tibble` (all part of the `tidyverse`) and `demodelr`. See @sec-packages-02 on how to install packages.  Prior to running any code in this section, include `library(tidyverse)` and `library(demodelr)`. 
:::

## The flu and locally linear approximation 
Consider @eq-flu-model-02, which is one way to model the rate of change of the flu through a population:

$$
\frac{dI}{dt} = 3e^{-.025t}
$$ {#eq-flu-model-02}

In @eq-flu-model-02 the variable $I$ represents the number of people infected at day $t$. One question we could address using @eq-flu-model-02 is to predict the value of $I$ after 1 day, assuming that $I(0)=10$. To do that we will build a locally linear approximation at $t=0$ and use the approximation to forecast and estimate $I(1)$.

In order to solve this problem, the formula for the locally linear approximation to $I(t)$ at $t=0$ is $L(t) = I(0) + I'(0) \cdot (t-0)$. Here, $I(0)=10$ and $I'(0)=3$ (found by evaluating @eq-flu-model-02 at $t=0$). Using $L(t) \approx I(t)$, the formula for the locally linear approximation is given by @eq-flu-linear-04. To define @eq-flu-linear-04 we used two pieces of information: the (given) value of the function at $t=0$ and the estimate of the derivative from @eq-flu-model-02.
...
At $t=1$ we can make a prediction with @eq-flu-linear-04 to estimate that there will be 13 people sick. To evaluate this approximation it is helpful to compare our prediction from $L(1)$ (@eq-flu-linear-04) to the actual value from the solution to the differential equation given in @eq-flu-exact-04:

$$
I(t) = 130-120e^{-.025t}
$$ {#eq-flu-exact-04}

@tbl-compare-04 compares the values of the linear approximation (@eq-flu-linear-04) to @eq-flu-exact-04:
...
Comparison of the exact solution $I(t)$ (@eq-flu-exact-04) to the linear approximation $L(t)$ (@eq-flu-linear-04) at $t=0$ and $t=1$. 

:::

@tbl-compare-04 shows that $L(1)$ is an *overestimate* compared to $I(1)$. Let's expand @eq-flu-linear-04 even more by constructing _another_ linear approximation using the differential equation. We will denote this linear approximation as $L_{1}(t)$ to distinguish it from $L(t)$ from @eq-flu-linear-04. First we evaluate @eq-flu-model-02, which yields $I'(1)=2.92$. The formula for the linear approximation at $t=1$ is $L_{1}(t) = I(1) + I'(1) \cdot (t-1)$. Here we will use $I(1) = 13$, recognizing that this value is a pretty close estimate for the number infected ($I$) at $t=1$. This assumption yields $L_{1}(t) = 13 +2.92(t-1)$. 

We can continue to build out the solution in a similar manner to develop a locally linear approximation at $t=2$, shown graphically in @fig-eulers-ver1. The approximation and the exact solution in @fig-eulers-ver1 appear very close to each other, suggesting that approximation using local linearization could work for other types of differential equations.


```{r, fig.cap='Approximation of a solution to @eq-flu-model-02 using local linearity.',echo=FALSE}
#| label: fig-eulers-ver1
...
tibble(
  ...
  ggthemes::scale_color_colorblind(labels = c("Linear approximation", "Exact solution")) 

```

By eye, the approximate and exact solutions in  @fig-eulers-ver1 appear indistinguishable from each other. Encouraged by these results, let's develop the approach with linear approximations even more.
...
The results of continuing this workflow (approximate $\rightarrow$ forecast $\rightarrow$ repeat) several times is shown in @tbl-compare-later-04. 
...
Calculation of the solution $I(t)$ for @eq-flu-model-02 using the linear approximations at each timestep with $\Delta t = 0.5$. 

:::

Notice how @tbl-calculate-forecast-04 organizes a way to compute the solution $I$ with linear approximations. Each row is a "step" of the method, computing the solution based on our step size $\Delta t$. The third column computes the value of the derivative for a particular time (@eq-flu-model-02), and then the fourth column represents the forecasted change in the solution by the next timestep.^[When you have a *rate of change* multiplied by a time increment this will give you an approximation of the net change in a function.]


This idea of *approximate, forecast, repeat* is at the heart of many [numerical methods](https://en.wikipedia.org/wiki/Numerical_methods_for_ordinary_differential_equations) that approximate solutions to differential equations. The particular method that we have developed here is called _Euler's method_. We display the results from additional steps in @fig-eulers-ver2. Based on the trend of the solution in @fig-eulers-ver2, it appears that the number of infections might start to level off at $I=130$, which is the steady state value in @eq-flu-exact-04 when evaluating $\displaystyle \lim_{t \rightarrow \infty} I(t)$.

```{r,fig.cap='Longer-term approximation of a solution to @eq-flu-model-02. Notice how the solution seems to level off to a steady state at $I=130$ (dashed line).',echo=FALSE}
#| label: fig-eulers-ver2
...
Now that we have worked on an example, let's carefully formulate Euler's method with another example. In @sec-intro-01 we discussed the spread of Ebola through a population. @eq-flu-de-04 is the differential equation for the logistic model (see @eq-all3-01 and @fig-three-soln), modeled with @eq-flu-de-pre-04:

$$
\frac{dI}{dt} = 0.023 I \cdot (13600-I),
$$ {#eq-flu-de-pre-04}


where the variable $I$ represents the proportion of people that are infected. The *carrying capacity*, or the place where the solution levels off in @eq-flu-de-pre-04 is at $I=13600$ (notice that when $I=13600$, $\displaystyle \frac{dI}{dt}=0$). Numerical methods such as Euler's method can become unstable for large values of the independent variable, because the rates are so large. To account for this, we will re-define @eq-flu-de-pre-04 with the variable $p = \frac{I}{13600}$, leading to the revised model:

$$
\frac{dp}{dt} = 0.023 p \cdot (1-p),
$$ {#eq-flu-de-04}
...
In @eq-flu-de-04 we define the function $f(p) = 0.023 p\cdot (1-p)$. In order to numerically approximate the solution, we will need to recall some concepts from calculus. This first step is that we will approximate the rate of change $\displaystyle \frac{dp}{dt}$ with a difference quotient (@eq-flu-dq-04):

$$
\frac{dp}{dt} = \lim_{\Delta t \rightarrow 0} \frac{p(t+\Delta t) - p(t)}{\Delta t}
$$ {#eq-flu-dq-04}

When the quantity $\Delta t$ in @eq-flu-dq-04 is small (for example $\Delta t = 1$ day), this difference quotient provides a reasonable way to organize the problem:
...
```{r echo=FALSE,fig.cap="Results from applying an iterative method to solve @eq-flu-de-04."}
#| label: fig-my-iterative-method
...
# Plot your solution:
ggplot(data = solution_data) +
  geom_line(aes(x = time, y = prop_infected),linewidth=1) +
labs(
  x = "Time (days)",
  y = "Proportion infected"
) +
...
Let's break the code down that produced @fig-my-iterative-method step by step:^[You may notice that when you run the code to produce @fig-my-iterative-method on your own it may not look like the output shown here. I've customized how the plots are displayed in this book using the pacakge `ggthemes`.]
...
- As before we need to identify $\Delta t$ (`deltaT`) and the number of steps $N$ (`n_steps`). When we generated the solution in @fig-my-iterative-method, in the `for` loop we defined the ending point at $t=2$ so the number of steps (`N`) was 20.^[In general if we know $\Delta t$ and the time we wish to end computing ($t_{end}$, then $N = t_{end}/\Delta t$.]
...
Now that we have some experience with Euler's method, let's see how we can apply the function `euler` to a system of differential equations. Here is a sample code that shows the dynamics for the lynx-hare equations, as studied in @sec-modeling-rates-03:
...
The variables $H$ and $L$ are already in thousands of animals, so we don't need to rescale anything like we did with @eq-flu-de-04. We are going to use Euler's method to solve this differential equation, using the code below:
...
```{r,warning=FALSE,echo=FALSE,fig.cap="Euler's method solution for Lynx-Hare system (@eq-lynx-hare-04)."}
#| label: fig-euler-system
...
out_solution |> pivot_longer(cols=c(-"t")) |>
  ggplot() +
  geom_line(aes(x = t, y = value,linetype=name,color=name), linewidth=1) +
  labs(
    x = "Time",
  ...
  ggthemes::scale_color_colorblind(labels = c("Hares", "Lynx"),name=NULL) 
```
...
- Most importantly, @eq-lynx-hare-04 has parameters, which we define as a named vector `lynx_hare_params <- c(r = 2, b = 0.5, e = 0.1, d = 1)` that we pass through to the command `euler` with the option `parameters`. If your equation does not have any parameters you do not need to worry about specifying this input.
...
Sometimes when working with Euler's method you encounter a differential equation that produces some nonsensible results. For example, consider a model that represents infection with quarantine (see @exr-flu-quarantine-01 in @sec-intro-01):
...
\end{split}
$$ {#eq-flu-quarantine-04}

In @eq-flu-quarantine-04, susceptibles become sick by encountering an infected person, but infected people are removed from the population at a rate $\beta$. The model in @fig-euler-flu-bad-04 illustrates the results when this model is implemented using `euler`:

```{r,echo=FALSE,fig.cap="Surprising results when using Euler's method to solve Equation \\@ref(eq:flu-quarantine-04). Notice how some values for $I$ are negative."}
#| label: fig-euler-flu-bad-04
...
```{r,echo=FALSE,fig.cap="Runge-Kutta solution for @eq-flu-quarantine-04. Notice how the solution curve for the variable $S$ does not fall below zero as it does in @fig-euler-flu-bad-04."}
#| label: fig-rk4-flu-good-04
...
where in @eq-de-general-04 $\vec{y}$ is the vector of state variables you want to solve for, and $\vec{\alpha}$ is your vector of parameters. At a given initial condition, Euler's method applies locally linear approximations to forecast the solution forward $\Delta t$ time units (@eq-euler-general-04):

$$
\vec{y}_{n+1} = \vec{y}_{n} + f(\vec{y}_{n},\vec{\alpha},t_{n}) \cdot \Delta t
$$ {#eq-euler-general-04}
...
::: {#exr-rk4-obs-04}
Apply the `rk4` solver with $\Delta t = 1$ with $N=600$ to the initial value problem $\displaystyle \frac{dp}{dt} = 0.023 p \cdot (1-p)$, $p(0)=250/13600$. Compare your solution to @fig-my-iterative-method. What differences do you observe?  Which solution method (`euler` or `rk4`) is better (and why)?
:::

::: {#exr-constant-04}
In the model presented by @eq-flu-quarantine-04, is $S+I$ constant? *Hint: add $\displaystyle \frac{dS}{dt}$ and $\displaystyle \frac{dI}{dt}$.*
:::
...
::: {#exr-step-de-04}
For each of the following differential equations, apply the Runge-Kutta method to generate a numerical solution to the differential equation and plot your solution. The stepsize ($\Delta t$) and number of iterations ($N$) are listed. Contrast your answers with @exr-euler-solve.
...
::: {#exr-rk4-euler}
Building on @exr-euler-rk4, let's say for a particular differential equation we have $N$ steps from $0 \leq t \leq b$. An error of $\epsilon$ is desired. 
...
a.  What is this model's is the steady state value?
b.  Apply the `rk4` method to determine solutions when $S(0)=0.2$ and $S(0)=0.8$ (two separate solutions) when $U = 1$, $r=0.1$, $M=0.7$, and $\lambda = 0.05$.
c.  Once that is complete, plot (with `ggplot`) your solutions with the initial conditions on the same plot. Be sure to label your axes!
d.  How do your solutions compare to the steady-state value you determined?
