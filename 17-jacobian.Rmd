...
In Sections \@ref(linearsystems-15) and \@ref(nonlinear-16) we focused on systems of differential equations and the types of behaviors we should expect when analyzing the stability of the equilibrium solution.  In this section we will analyze stability of a nonlinear system through the concept of *local linearization*, beginning to build a bridge between nonlinear and linear systems of equations.
...
So, at the equilibrium solution $(0,0)$, Equation \@ref(eq:lynx-hare-17)  behaves like the following system of equations:

\begin{align}
\frac{dH}{dt} &= .3H\\
\frac{dL}{dt} &= - .2L (\#eq:lynx-hare-17-00)
\end{align}


Notice how Equation \@ref(eq:lynx-hare-17-00) is an entirely decoupled linear system of equations that can be solved easily by separation of variables. For an initial condition $(H_{0},L_{0})$ Equation \@ref(eq:lynx-hare-17-00) has a solution $H(t)=H_{0}e^{.3t}$ and $L(t)=L_{0}e^{-.2t}$.

## The Jacobian matrix
RevisitingEquation \@ref(eq:lynx-hare-17-00) we can write our system in matrix form:
...
\frac{dL}{dt} &=ebHL -dL
\end{split} (\#eq:logistic-system-17)
\end{equation}

Through a rescaling of Equation \@ref(eq:logistic-system-17) with the variables $\displaystyle x=\frac{H}{K}$, $\displaystyle y=\frac{L}{r/b}$ and $T = r t$ we can rewrite Equation \@ref(eq:logistic-system-17) as:

\begin{equation}
\begin{split}
\frac{dx}{d T} &= x(1-x) - xy \\
\frac{dy}{d T} &=\frac{ebK}{r}xy -\frac{d}{r}y
\end{split} (\#eq:logistic-system-rev-17)
\end{equation}

In order to analyze the Jacobian matrix for Equation \@ref(eq:logistic-system-rev-17) we will need to compute several partial derivatives:
...
c. With the Jacobian matrix, visualize a phaseplane at these equilbrium solutions to determine stability.
...
c. With the Jacobian matrix, visualize a phaseplane at that equilbrium solution to determine stability.
...
b. Determine the Jacobian matrix at this equilbrium solution.
c. With the Jacobian matrix, visualize a phaseplane at that equilbrium solution to determine stability.
...
c. With the Jacobian matrix, visualize a phaseplane at that equilbrium solution to determine stability.
d. Use the fact that that $\displaystyle \frac{dy}{dx} / \frac{dy}{dt} = \frac{dy}{dx}$, which should yield a separable differential equation that will allow you to solve $y(x)$.  How does that solved differential equation compare to what you found by analyzing the Jacobian matrix?
...
a. Verify that the point $\displaystyle \left( \frac{d}{eb}, \frac{r}{b} \right)$ is an equilibrium solution.
...
a. Verify that by rescaling of this system $\displaystyle x=\frac{H}{K}$, $\displaystyle y=\frac{L}{r/b}$ and $T = r t$ we can rewrite this system as:
\begin{equation}
\begin{split}
\frac{dx}{d T} &= x(1-x) - xy \\
\frac{dy}{d T} &=\frac{ebK}{r}xy -\frac{d}{r}y
...
```{exercise}
(Inspired by @logan_mathematical_2009) A model for the spread of a disease where people recover is given by the following differential equation:
...
a. Determine all the equilibrium solutions for this model.
b. Construct the Jacobian matrix for each of the the equilibrium solutions.
c. Select different parameter values.  With the Jacobian, construct a phase plane diagram for each of the equilibrium solutions.
d. In the absence of fishing $E=0$.  How do these equilibrium solutions compare to the ones from the previous part?
