...
## Ensemble Averages
Consider Figure \@ref(fig:kuopio-weather), which shows the weather forecast for Kuopio, a city in Finland:^[I could have picked any city, but a lot of this textbook was written while I was on sabbatical in Kuopio. I highly recommend Finland as a country to visit.]

```{r kuopio-weather, echo=FALSE,out.width = "50%",fig.align="center",fig.cap = "Long-term weather forecast for Kuopio, a city in Finland, from the [https://en.ilmatieteenlaitos.fi/](Finnish Weather Institute)"}
knitr::include_graphics("figures/22-simulation/kuopio.png")
```

Figure \@ref(fig:kuopio-weather) shows a great example of what is called an *ensemble average*. The horizontal axis lists the time of day and the vertical axis is the temperature (the precipitation is in bars down below, but not important in this context). The forecast temperature at a given point in time can have a range of outcomes, with the median of the distribution as the "temperature probability forecast"[^stochasticsimulation-1]. The red shading states that 80% of the outcomes fall in a given range, so while the median temperature on Tuesday May 5 (labeled as Tu 4.5 - dates are represented as DAY-MONTH-YEAR) is 7 $^{\circ}$C, it may range between 4 and 11 $^{\circ}$C (39 to 51 $^{\circ}$F). Based on the legends given, the 80% confidence interval is between 4 - 11 $^{\circ}$C, or the models have 80% confidence for the temperature to be between that range of temperatures.
...
In this section we will focus on how to take results from a simulation (such as several simulations of the moose population from Section \@ref(stoch-sys-21)) to generate ensemble average plots. Computing an ensemble average is a great way to show your uncertainty or confidence in a model outcome. 

## Computing ensemble averages
Let's build up how to compute an ensemble average step by step. For example let's say we have the following data in Table \@ref(tab:simul-table). Notice how all the simulations (`sim1`, `sim2`, `sim3`) share the variable `t` in common, so it makes sense to plot them on the same axis in Figure \@ref(fig:simul-graph). We call a plot of all of the simulations together a *spaghetti plot*, because it can look like a bowl of spaghetti noodles was dumped all over the plot.
...
```{r simul-graph-prev,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Initial Timeseries plot of the three simulations from Table \\@ref(tab:simul-table)."}
my_table %>%
  ggplot() +
...
One thing to adjust in the code used to produce the spaghetti plot in Figure \@ref(fig:simul-graph-prev) is that the code is really long - we needed a `geom_point` and `geom_line` for each simulation.  While this isn't bad when you have three simulations, one lesson we will learn is that the more simulations gives you better confidence in your results. If we have 500 simulations this would be a pain to code! Fortunately there is a command called `pivot_longer` that gathers different columns together (Table \@ref(tab:simul-table-long).
...
```{r simul-graph,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Timeseries plot of the three simulations from the pivoted data frame `my_table_long`."}
my_table_long %>%
...
```{r simul-graph-rev,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Timeseries plot of the three simulations from Table \\@ref(tab:table-summary).",message=FALSE,warning=FALSE}
my_table_long %>%
  ggplot(aes(x = t, y = value, group = name)) +
  geom_point() +
  geom_line() +
...
```{r simul-graph-conf,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Timeseries plot of the three simulations, along with the 95\\% confidence interval (red)."}
my_table_long %>%
...
```{r logistic-example,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Solution to the logistic differential equation."}
...
```{r logistic-example-rev,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Solution to the logistic differential equation with a random initial condition (red). The original solution is also plotted for comparison."}
...
```{r logistic-example-many,fig.width = 4,fig.height=3,fig.align="center",fig.cap="Many realiziations of the logistic differential equation with random initial conditions.",fig.show='asis'}
