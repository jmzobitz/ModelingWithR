# Phase Lines and Equilibrium Solutions {#sec-phase-05}

@sec-euler-04 explored numerical techniques to solve initial value problems. This chapter takes a step back to examine the general family of solutions to a differential equation. Will the family of solutions converge in the long run (as $t \rightarrow \infty$) to a constant value? Are these specific solutions that always remain constant (or independent of time)? Answering questions such as these address the *qualitative* behavior for a single differential equation.^[Qualitative behavior for coupled systems of differential equations is addressed in @sec-coupled-06.] Let's get started!\index{differential equation!qualitative behavior}

## Equilibrium solutions
@sec-modeling-rates-03 introduced the concept of an equilibrium solution, or where the rate of change for a differential equation is zero.\index{equilibrium solution} We can determine equilibrium solutions for a single-variable differential equation by setting the left hand side of $\displaystyle \frac{dy}{dt}=f(y)$ equal to zero and solving for $y$ (or whatever dependent variable describes the problem).
...
The general solution to the differential equation $\displaystyle \frac{dy}{dt}=- y$ is $y(t)=Ce^{-t}$, where $C$ is an arbitrary constant. (We will explore techniques to determine this in @sec-exact-solns-07.)  @fig-exponential plots different initial conditions, with the equilibrium solution shown as a horizontal line:

```{r, echo=FALSE,fig.cap="Solution curves to $y'=-y$ for different initial conditions (values of $C$)."}
#| label: fig-exponential

t <- seq(0, 5, length.out = 50)
data.frame(time = t, s1 = 2* exp(-t), s2 = 1* exp(-t), s3 = -1 * exp(-t), s4 = -2 * exp(-t)) |>
  tidyr::pivot_longer(names_to = "constant", values_to = "soln", cols=c(-"time")) |>
  ggplot() +
  geom_line(aes(x = time, y = soln, color = constant), linewidth = 1) +
  theme_bw() +
  theme(
  ...
  ggthemes::scale_color_colorblind(
    labels = c("C = 2", "C = 1", "C = -1", "C = -2"),
    name = NULL
  )
```


Notice that in @fig-exponential as $t$ increases, all solutions approach the equilibrium solution $y=0$, regardless if the initial condition is positive or negative. This observation is also confirmed by evaluating the limit $\displaystyle \lim_{t\rightarrow \infty} Ce^{-t}$, which is 0.
...
::: {#sol-logistic-05}
In this case the equilibrium solutions for @eq-logistic-05 occur when $N \cdot(1-N) = 0$, or when $N=0$ or $N=1$.

The general solution to @eq-logistic-05 is

$$
N(t)= \frac{C}{C +(1-C) e^{-t}}.
$$ {#eq-logistic-soln-05}

@fig-logistic-soln displays several different solution curves for @eq-logistic-soln-05.
:::


```{r, echo=FALSE,fig.cap="Solution curves for @eq-logistic-05 with different initial conditions (values of $C$)."}
#| label: fig-logistic-soln
...
data.frame(time = t, s1 = 2 / (2 + (1 - 2) * exp(-t)), s2 = 1.5 / (1.5 + (1 - 1.5) * exp(-t)), s3 = 0.5 / (0.5 + (1 - 0.5) * exp(-t)), s4 = 0.25 / (0.25 + (1 - 0.25) * exp(-t)), s5 = 0.1 / (0.1 + (1 - 0.1) * exp(-t))) |>
  pivot_longer(names_to = "constant", values_to = "soln", cols=c(-"time")) |>
  ggplot() +
  geom_line(aes(x = time, y = soln, color = constant), linewidth = 1) +
  theme_bw() +
  theme(
...
In @fig-logistic-soln notice how all the solutions tend towards $N=1$, but even solutions that start close to $N=0$ seem to move away from the $N=0$ equilibrium solution. Solutions in  @fig-logistic-soln exhibit the idea of the *stability* of an equilibrium solution, which we discuss next.
...
Let's apply this logic to our differential equation $\displaystyle \frac{dy}{dt}=- y$ from @exm-exponential-05. When $y=2$, $\displaystyle \frac{dy}{dt}=- 2 <0$, so we say the function is *decreasing* to $y=0$. When  $y=-2$, $\displaystyle \frac{dy}{dt}=- (-2) = 2 >0$, so we say the function is *increasing* to $y=0$. This can be represented neatly in the *phase line diagram* for @fig-phaseline1.^[Sometimes arrows are used in the phase line to signify if the solutions are increasing or decreasing. I will stick to the convention presented in @fig-phaseline1 because it illustrates connections between the differential equation and the solution.]


```{r,echo=FALSE,fig.cap="Phase line for the differential equation $y'=-y$."}
#| label: fig-phaseline1
...
Because the solution is *increasing* to $y=0$ when $y <0$, and *decreasing* to $y=0$ when $y >0$, we say that the equilibrium solution for the differential equation $y'=-y$ is **stable**, which is also confirmed by the solutions plotted in @fig-exponential.
...
- When $r<0$, the phase line will be similar to @fig-phaseline1.
- When $r>0$ the phase line will be as shown in @fig-phaseline2. We say in this case that the equilibrium solution is *unstable*, as all solutions flow away from the equilibrium. Several different solutions are shown in @fig-solncurve-1 .
- When $r=0$ we have the differential equation $\displaystyle \frac{dy}{dt}=0$, which has $y=C$ as a general solution. For this special case the equilibrium solution is neither stable or unstable^[Arguably when $r=0$ the resulting differential equation $y'=0$ is different than $y'=ry$; something peculiar is going on here - which is discussed more in @sec-bifurcation-20.].


  
```{r,echo=FALSE, fig.cap="Phase line for the differential equation $y'=ry$, with $r>0$."}
#| label: fig-phaseline2
eq_soln <- data.frame(x = c(0), y = c(0), label = "y = 0")
...
```{r, echo=FALSE,fig.cap="Solution curves for the differential equation $y'=ry$, with $r>0$ for different initial conditions (values of $C$)."}
#| label: fig-solncurve-1
...
Based on the above discussion, let's return back to the differential equation $\displaystyle \frac{dN}{dt} = N \cdot(1-N)$ from @exm-logistic-05). We evaluate the stability of the equilibrium solutions $N=0$ and $N=1$ in @tbl-logistic-table-05.

::: {#tbl-logistic-table-05}
...
Evaluation of the stability of the equilibrium solutions for @eq-logistic-05.

:::

Notice how the selected test points in the first column of @tbl-logistic-table-05 were selected to the _left_ or the _right_ of the equilibrium solutions. The phase line diagram of @fig-phaseline-N also presents the same information as in @tbl-logistic-table-05, but in contrast to @fig-phaseline1 and @fig-phaseline2 we need to include _two_ equilibrium solutions. Phase line diagrams should include all the computed equilibrium solutions.

  
```{r,echo=FALSE,fig.cap="Phase line diagram for @eq-logistic-05)."}
#| label: fig-phaseline-N
...
Consider the general differential equation $\displaystyle \frac{dy}{dt}=f(y)$ with an equilibrium solution at $y_{*}$. Next we apply local linearization to construct a locally linear approximation $L(y)$ for $f(y)$ at $y=y_{*}$ (@eq-local-linear-1-05):

$$
L(y) = f(y_{*}) + f'(y_{*}) \cdot (y-y_{*}) 
$$ {#eq-local-linear-1-05}

There are two follow-on steps to simplify @eq-local-linear-1-05. First, because we have an equilibrium solution, $f(y_{*}) =0$. Second, @eq-local-linear-1-05 can be written with a new variable $P$, defined by variable $P = y-y_{*}$. With these two steps @eq-local-linear-1-05 translates to @eq-local-linear:

$$
\frac{dP}{dt} = f'(y_{*}) \cdot P
$$ {#eq-local-linear}

Does @eq-local-linear look familiar?  It should!  This equation is similar to the example where we classified the stability of $\displaystyle \frac{dy}{dt} = ry$ (notice that $f'(y_{*})$ is a number). Using this information, a test to classify the stability of an equilibrium solution is the following:
...
Let's return back to the differential equation $\displaystyle \frac{dN}{dt} = N \cdot(1-N)$ from @exm-logistic-05 and apply the local linearization stability test, $f'(N)=1-2N$. Since $f'(0)=1$, which is greater than 0, the equilibrium solution $N=0$ is unstable. Likewise, if $f'(1)=-1$, the equilibrium solution $N=1$ is stable.

Applying the local linearization test may be easier to quickly determine stability of an equilibrium solution. Guess what? This test also is a simplified form of determining stability of equilibrium solutions for systems of differential equations. We will explore this more in @sec-stability-19.
...
::: {#exr-phase-prev-05}
Using your results from @exr-eq-soln-ex, construct a phase line for each of the differential equations and classify the stability of the equilibrium solutions.
:::
...
::: {#exr-another-cell-05}  
{{< fa burst >}} A cell with length $L$ assimilates nutrients at a rate proportional to its surface area, but uses nutrients proportional to its volume, according to the following differential equation:
...
::: {#exr-advert-phase-05}
{{< fa burst >}} The steady state for the differential equation $\displaystyle \frac{dS}{dt} = \frac{rU}{M}(M-S) - \lambda S$ is $\displaystyle S_{*} = \frac{rUM}{rU+\lambda M}$. Assume $r>0$, $U>0$, $M>0$, and $\lambda>0$. Apply the local linearization stability test to determine the stability of this equilibrium solution.
:::
