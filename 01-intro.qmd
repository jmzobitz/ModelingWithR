# Models of Rates with Data  {#sec-intro-01}
...
ebola |>
  dplyr::filter(nation == "Cases Sierra Leone") |>
  ggplot() +
...
@eq-00infected is an example of a *differential equation*\index{differential equation}, which is just a mathematical equation with rates of change. In @eq-00infected $k$ is a proportionality constant or parameter\index{parameter!differential equation}, with units of time$^{-1}$ for consistency.

The *solution* to a differential equation is a function $I(t)$. When we "solve" a differential equation we determine the family of functions consistent with our rate equation.^[You may be used to working with *algebraic equations* (e.g. solve $x^{2}-4=0$ for $x$) rather than differential equations. For algebraic equations the solution can be points (for our example, the solution to $x^2-4=0$ is $x=\pm2$).] There are a lot of techniques to solve a differential equation; we will explore some in @sec-exact-solns-07.

The proportionality constant or *parameter* $k$ is important to understand the solution to @eq-00infected. Even though no numerical value for $k$ is specified, you can always solve an equation without specifying the parameter. In some situations we may not be as concerned with the particular _value_ of the parameter but rather its influence on the long-term behavior of the system (this is a key aspect of bifurcation theory\index{bifurcation} described in @sec-bifurcation-20. Otherwise we can use the collected data shown above with the given model to determine the value for $k$. This combination of a mathematical model with data is called *data assimilation* or *model-data fusion* (see Chapters [-@sec-linear-regression-08]-[-@sec-information-criteria-14]).


How plausible is this first model? The first model assumes the rate of change (@eq-00infected) gets larger as the number of infected people $I$ increases. This reasoning certainly seems plausible: when there are so many people infected it can be hard to stay healthy! At some point the number of people who are *not* sick will reach zero, making the rate of infection zero (or no increase). In the case of Ebola or any other infectious disease, stringent public health measures would be enacted if the number of people infected became too large.^[The COVID-19 pandemic that began in 2020 is an example of the heroic efforts of public health officials.] Following public health measures we would expect that the rate of infection would decrease and the number of infections to slow. So perhaps another model this can capture this "slowing down" of the infection rate is more plausible. 

### Model 2:  Infection rate proportional to number NOT infected
The second model considers the interaction between people who are sick (which we have denoted as $I$) and people who are *not* sick, which we will call $S$, or susceptible.\index{susceptible} @eq-01notinfected is an example of a differential equation that models this interation:

$$ 
\frac{dI}{dt} = kS
$$ {#eq-01notinfected}

As with @eq-00infected the parameter $k$ represents an infection rate. We would expect that both $I$ and $S$ change in time as the infection occurs; for a finite population as more people get sick ($I$), that would mean that $S$ would decrease. In effect, Model 2 should have *two* rates of change: one for $I$ and one for $S$. @fig-initial-si shows a schematic of this process of infection.

```{tikz, label=fig-initial-si,fig.cap="Schematic diagram for Model 2, showing that the rate of infection is proportional to the number of susceptible people $S$. Assuming a constant population size $N$, the differential equation for Model 2 is given by @eq-single-02.",echo=FALSE,message=FALSE}
...
There are three reasons why I like to use diagrams like @fig-initial-si:
...
(3) Diagrams help to identify assumed parameters (i.e. $k$ in @fig-initial-si).
(4) Diagrams suggest how to construct differential equations for this mathematical model. @fig-initial-si suggests a flow between the suspectible state $S$ to the infected state $I$. So then the rate of change equation for $S$ is $\displaystyle \frac{dS}{dt} = -kS$ (the parameter listed above the arrow in @fig-initial-si). @eq-02coupled combines all this thinking and @eq-01notinfected into the following coupled system of differential equations in @eq-02coupled:
...
The solution to @eq-02coupled is functions $S(t)$ and $I(t)$ that evolve over time. We don't have the tools to determine the exact solutions for @eq-02coupled yet (we will study systems like these in Chapters [-@sec-linearsystems-15]-[-@sec-bifurcation-20]). However something interesting occurs with @eq-02coupled when we add the rates $\displaystyle \frac{dS}{dt}$ and $\displaystyle \frac{dI}{dt}$ together (@eq-single-02):

$$
\frac{dS}{dt} +  \frac{dI}{dt} = \frac{d}{dt}(S+I) = 0
$$ {#eq-single-02}


If a rate of change equals zero then the corresponding function is constant. In effect, @eq-single-02 means that the combined variable $S+I$ is constant, so we could say that $S+I=N$, where $N$ is the total population size. The expression $S+I=N$ is an example of a conservation law\index{conservation law} for our system.^[A finite population (meaning nobody can exit or enter the population) usually should have some type of conservation law.] @fig-initial-si also suggests a conservation law because there are no additional arrows going into or from the variables $S$ or $I$. Since $S=N-I$, @eq-02coupled can be re-written with a single equation (@eq-02single):
...
Notice how in @fig-logistic-scheme there is an additional variable $S$ associated with $k$ to show how the rate of infection depends on $S$. @eq-logistic-sys-01 contains the differential equations that describe the scenario outlined in @fig-logistic-scheme:
...
Similar to Model 2 we can combine the two equations to yield a single differential equation (@eq-logistic-01):

$$
\frac{dI}{dt} = k\cdot I \cdot (N-I)
$$ {#eq-logistic-01}

@eq-logistic-01 appears similar to @eq-02single, doesn't it?  However in @eq-logistic-01 notice the variable $I$ outside the expression $(N-I)$. If $I=0$, then there is no increase in infection (the rate is zero). If $I=N$ (the total population size) then there is no increase in the infection (the rate is zero as well). Model 3 seems to be more consistent with the biological reasoning for the spread of infection.


Let's compare all the rates for all three models together in @fig-threeRates-01. @fig-threeRates-01 has a lot to unpack, but we can use some of our understanding of rates of change in calculus to compare the three models. Notice how the sign of $\displaystyle \frac{dI}{dt}$ is always positive for Model 1, indicating that the solution ($I$) is always increasing. For Models 2 and 3, $\displaystyle \frac{dI}{dt}$ equals zero when $I=10$, which also is the value for $N$  After that case, $\displaystyle \frac{dI}{dt}$ turns negative, meaning that $I$ is decreasing.

```{r,fig.cap='Comparing the rates of change for three models (@eq-00infected, @eq-02single, and @eq-logistic-01), with $k=1$ and $N=10$).',echo=FALSE,message=FALSE}
#| label: fig-threeRates-01
...
data.frame(x, m1 = k*x, m2 = k*(n - x), m3 = k*x * (n - x)) |>
  pivot_longer(names_to = "model", values_to = "rate", cols=c(m1, m2, m3)) |>
  ...
  ggthemes::scale_colour_colorblind(labels = c("Model 1", "Model 2", "Model 3"), name = NULL)
...
Notice how I assigned the names to each model (Exponential, Saturating, and Logistic). That may not mean much at the moment, but @fig-three-soln) plots the three functions $I(t)$ together when $I_{0}=250$, $k=0.023$, and $N=13600$.

```{r,label=fig-three-soln,message=FALSE,echo=FALSE,warning=FALSE,fig.cap='Three models (Exponential, Saturating, and Logistic; @eq-all3-01) compared.'}
...
data.frame(x, m1 = i0 * exp(k * x), m2 = n - (n - i0) * exp(-k * x), m3 = n * i0 / (i0 + (n - i0) * exp(-k * x))) |>
  pivot_longer(cols=c(m1,m2,m3),names_to= "model", values_to = "rate") |>
  ggplot(aes(x = x, y = rate, color = model)) +
  geom_line(linewidth = 1) +
  xlab("t (days)") +
  ylab("Infected people") +
  theme_bw() +
  ggthemes::scale_color_colorblind(labels = c("Model 1", "Model 2", "Model 3"), name = NULL) +
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 14),
    axis.title.x = element_text(size = 14),
...
Notice how in @fig-three-soln, Model 1 increases quickly - it actually grows without bound off the chart!  Model 2 and Model 3 have saturating behavior, but it looks like Model 3 might be the one that actually captures the trend of the data. 
...
We will address several of these critera later on in this textbook when we discuss *model selection* (@sec-information-criteria-14).\index{model!selection}  Model selection is one key part of the modeling hypothesis - where we investigate the implications of a particular model analyzed. If we don't do this, we don't have an opportunity to test out what is plausible for our models.
...
::: {#exr-liberia-n-01}
The Ebola outbreak for the country of Liberia in 2014 is shown in @fig-out-liberia.
If we were to apply the logistic model (Model 3) based on this graphic what would be your estimate for $N$?
:::



```{r,label=fig-out-liberia,fig.cap='Infections from a 2014 Ebola outbreak in Liberia, with the initial monitoring in March 2014.',echo=FALSE,message=FALSE}

ebola |>
  dplyr::filter(nation == "Cases Liberia") |>
  ggplot() +
  geom_point(aes(x = monitor_days, y = cases), size = 1) +
  theme_bw() +
  theme(
...
\end{split}
$$
where $I_{0}$ is the initial number of people infected and $N$ is the overall population size. Using the functions from @exr-plot-soln for both models, what are the values for $N$ and $I_{0}$?
:::

::: {#exr-general-specific-01}
The general solutions for the saturating and the logistic models are:
...
\end{split}
$$
where $I_{0}$ is the initial number of people infected and $N$ is the overall population size. For both models carefully evaluate the limits to show $\lim_{t \rightarrow \infty} I(t)=N$. How do these limiting values compare to the steady-state values you found for Models 2 and 3 in @fig-three-soln), where $N=13600$?
...
```{tikz, label=fig-exercise-imm,fig.cap="Schematic diagram for @exr-simple-mig-01.",echo=FALSE,message=FALSE}
\tikzstyle{vspecies}=[rectangle,minimum size=0.5cm,draw=black]
\begin{tikzpicture}[auto, outer sep=1pt, node distance=2cm]
...
::: {#exr-simple-mig-01}
@fig-exercise-imm shows a schematic diagram which is a variation on @fig-initial-si. In this case people are entering the the susceptible population $S$ at a rate $\beta$, so the population is not conserved. What is the coupled system of differential equations for this model?
:::
...
::: {#exr-new-rates-01}
{{< fa burst >}} The following figure shows a diagram in which people are entering the the infected population $I$ at a rate $\gamma$, so the total population size is not constant. What is the coupled system of differential equations for this model?
...
\node [vspecies] (S) {$S$} ;
\node [vspecies, right of = S] (I) {$I$} ;
\node [above of = I] (gamma) {} ;
\draw [->,thick] (S) --  node {\small{$k$}} (I) ;
\draw [->,thick] (gamma) --  node {\small{$\gamma$}} (I) ;
...
::: {#exr-adele-01}
u
{{< fa burst >}} The singer Adele was correct - ["Rumor has it."](https://youtu.be/A8IRqCJVkOo?si=yeBf6_KwyXptrojP) The spread of a rumor through a population can be modeled with a differential equation. The following differential equation describes the rate of change in the proportion of the people who know the rumor $R$ as the time $t$ increases (in hours:)

$$
\begin{split}
    \mbox{Saturating model: } & R(t) = 1-0.8e^{-0.6t} \\
    \mbox{Logistic model: } &  R(t) = \frac{0.2 }{0.2+0.8e^{-0.6t}},
\end{split}
$$

a.  For both models, what is the value long term value $N$ (i.e. $\displaystyle \lim_{t \rightarrow \infty} I(t)$)?
b.  What is the initial value $I_{0}$?
c.  Plot the saturating and logistic models and include a careful sketch in your solution, showing all the limiting behavior. Which model makes more sense to you?

:::


<!-- See hansen_long-term_2020 for excess herpes mortality, shkolnikov_what_2022 baseline mortality, ayoub_characterizing_2019 for incidence (beta) and  Jones-on-R0.pdf for background on the SI model.  -->

::: {#exr-herpes-01}
{{< fa burst >}} An important parameter in modeling the spread of diseases is the $R_{0}$, which represents if a disease will become an epidemic. If $R_{0}>1$, then the disease will become an epidemic. If $R_{0}<1$, then it will not. For a disease like Herpes, one model is a susceptible - infected ($SI$) model, assuming there is no recovery from Herpes. For this case, an expression for $R_{0}$ is the the following:\
...
a.  Let $\mu = \frac{1}{1000}$ and $\delta = \frac{1.193}{1000}$. Plot $R_{0}$ as a function $\beta$ and include your plot in the response. Make sure $0 \leq R_{0} \leq 2$. (You will need to find an appropriate window for $\beta$.). What is the threshold value for $\beta$ for an epidemic?
b.  Let $\mu = \frac{1}{1000}$ and $\beta = \frac{2}{100}$. Plot $R_{0}$ as a function $\delta$ and include your plot in the response. Make sure $0 \leq R_{0} \leq 2$. (You will need to find an appropriate window for $\delta$.). What is the threshold value for $\delta$ for an epidemic?
...
{{< fa burst >}} Another example of an advertising model is
...
a.  Solve $\displaystyle \frac{dS}{dt}$ for the steady state value (where $\displaystyle \frac{dS}{dt}=0$). Your final answers should be expressed in terms of the parameters. Show your work.
b.  Using your expression for the steady state value from (a), evaluate what happens to the steady state in the extreme cases when $M=0$ or $M=1$. Is it possible to get 100% of the market share ($S=1$) when you have the ability to have 100% of the market ($M=1$)?
c.  Using your expression for the steady state value from (a), evaluate what happens to the steady state as $U$ grows large (in other words, you spend as much on advertising as possible, or $U \rightarrow \infty$). Why does that answer make sense in the context of the problem?
:::
