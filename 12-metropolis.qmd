...
In @sec-r-intro-02 we introduced the dataset `wilson`, which reported data on the [weight of the puppy Wilson](http://bscheng.com/2014/05/07/modeling-logistic-growth-data-in-r/) as he grew, shown again in @fig-wilson-data-12. Because we will re-use the scatter plot in @fig-wilson-data-12 several times, we define the variable `wilson_data_plot` so we don't have to re-copy the code over and over.^[Making a base plot and repeatedly adding to it is a really good coding practice for `R`.]
...
```{r,fig.cap="Weight of the dog Wilson over time.",echo=FALSE}
#| label: fig-wilson-data-12
wilson_data_plot <- ggplot(data = wilson) +
  geom_point(aes(x = days, y = weight), size = 1, color = "red") +
  labs(
    x = "D (Days since birth)",
...
Let $D$ be the days since birth and $W$ Wilson's weight in pounds, @eq-wilson-sat-12 is a model that describes Wilson's weight over time:

$$
W =f(D,p_{1})= \frac{p_{1}}{1+e^{(p_{2}-p_{3}D)}}
$$  {#eq-wilson-sat-12}

@eq-wilson-sat-12 has three different parameters $p_{1}$, $p_{2}$, and $p_{3}$ to be estimated. For convenience we will set $p_{2}= 2.461935$ and  $p_{3} = 0.017032$ and estimate $p_{1}$, which for this model represents the long-term weight for Wilson, or a horizontal asymptote for @eq-wilson-sat-12 (@exr-limit-12).

Let's take an initial guess for the parameter $p_{1}$. From @fig-wilson-data-12 a reasonable guess for $p_{1}$ would be 78. As we did with @fig-wilson-data-12, we are going to store the updated plot as a variable. Try this code out on your own. 

```{r,fig.cap="Weight of the dog Wilson with initial guess $p_{1}=78$.",eval = FALSE}
#| label: fig-wilson-with-guess-12
...
Hopefully this code seems familiar to you from @sec-likelihood-09, but of note are the following:
...
Notice how we computed $L(65)$ and $L(78)$ separately and then compared the two values. Another approach is to examine the *ratio* of the likelihoods (@eq-like-ratio-12):
...
Applying @eq-like-ratio-12 with $p_{1}^{proposed}=65$ and $p_{1}^{current}=78$, we have $\displaystyle \frac{ L(65) }{ L(78) }=$ `r round(out_values$l_hood[[2]]/out_values$l_hood[[1]])`, further confirming $p_{1}=65$ is more likely compared to the value of $p_{1}=78$.

### Iterative improvement with likelihood ratios
We can improve on estimating $p_{1}$ for @eq-wilson-sat-12 by continuing to compute likelihood ratios. However, since $p_{1}=65$ is the more likely value (currently), then we will set $p_{1}^{current}=65$ for @eq-like-ratio-12.

To simplify things, let's define a function that will quickly compute @eq-wilson-sat-12 for this dataset:
...
I think we are onto something - @fig-all-three-wilson compares the modeled values of Wilson's weight for the different parameters:

```{r,fig.cap="Comparison of our three estimates for Wilson's weight over time.",echo=FALSE,warning=FALSE,message = FALSE}
#| label: fig-all-three-wilson
...
@sec-lr-12 outlined an iterative approach of applying likelihood ratios to estimate $p_{1}$. Let's organize all the work in a table (@tbl-mh-table-12).
...
We have done the Metropolis-Hastings algorithm "by hand", which may seem tedious to do, but it helps build your own iterative understanding of the underlying process. Here's the good news: we can easily automate the Metropolis-Hastings algorithm, which we will explore in @sec-mcmc-13. To note, there are several modifications we can do to make the Metropolis-Hastings algorithm a more efficient and robust method:


- While we have focused on implementation of the Metropolis-Hastings algorithm with one parameter, this is easily extended to sets of parameter values (e.g. estimating $p_{1}$, $p_{2}$, and $p_{3}$ in @eq-wilson-sat-12). However it may take longer to determine the global optimum because we change one parameter value at a time.
...
::: {#exr-wilson-more-12}
Using the dataset `wilson` from this chapter, complete 10 iterations of the Metropolis-Hastings algorithm by continuing @tbl-mh-table-12. See if you can get the value of $p_{1}$ to 2 decimal places of accuracy. Be sure to include a plot of the data and the model with the final estimated value of $p_{1}$.
:::
...
a. Apply separation of variables and $W(0)=5$ and the value of $p_{1}$ from @exr-wilson-more-12 to determine the solution for this differential equation.
