# Solutions to Stochastic Differential Equations {#sec-solvingSDEs-27}


Chapters [-@sec-sdes-25] and [-@sec-simul-stoch-26] simulated SDEs. Visualizing the ensemble average of solution trajectories revealed a *distribution* of solutions that evolves in time. This final chapter examines methods to characterize exact solutions to stochastic differential equations by developing formulas to characterize the evolution of the distribution of solutions in time. There are a lot of mathematics here that can be a stepping stone for further study in the field of stochastics. Let's (for one last time) get started!
...
The "solution" to this SDE will be a probability density function $p(x,t)$, describing the evolution of $p(x,t)$ in both time and space. Based on our work with Brownian motion in Chapters [-@sec-sdes-25] and [-@sec-simul-stoch-26], the probability density function $p(x,t)$ should have the following properties:
...
A way to determine the probability density function is by solving the following partial differential equation, termed the **Fokker-Planck Equation** (@eq-fpe-27).\index{Fokker-Planck equation}

$$
\frac{\partial p}{\partial t} = - \frac{\partial}{\partial x} \left(p(x,t) \cdot a(x,t) \right) + \frac{1}{2}\frac{\partial^{2} }{\partial x^{2}} \left(\; p(x,t) \cdot (b(x,t))^{2}  \;\right)
$$ {#eq-fpe-27}

We can write @eq-fpe-27 in shorthand, dropping the dependence of $x$ and $t$ for $p(x,t)$, $a(x,t)$ and $b(x,t)$: $\displaystyle p_{t} = -  (p \cdot a)_{x} + \frac{1}{2} (p \cdot b^{2})_{xx}$. We do not include the derivation of the Fokker-Planck Equation here, as the proof requires concepts from  advanced calculus (see @logan_mathematical_2009 for more discussion and derivation of the Fokker-Planck equation). However, let's build up understanding of @eq-fpe-27 through some examples.

### Diffusion (again)
Consider the SDE $dx = dW(t)$ with $x(0)=0$ and apply the Fokker-Planck equation to characterize the solution $p(x,t)$. We know from @sec-diffusion-24 that SDE $dx = dW(t)$ characterizes Brownian motion. When we compare this SDE to the Fokker-Planck equation and @eq-generic-sde-27, we have $a(x,t)=0$ and $b(x,t)=1$, yielding @eq-fpe-diff-27:

$$
p_{t} = \frac{1}{2} p_{xx}.
$$  {#eq-fpe-diff-27}

This equation should look familiar - it is the partial differential equation for diffusion (@eq-new-master-diff)!^[To remind you, the solution to $p_{t} = D p_{xx}$ is $\displaystyle p(x,t) = \frac{1}{\sqrt{4 \pi Dt} } e^{-x^{2}/(4 D t)}$. So in this case $D = 1/2$.]  The solution to this SDE is given by @eq-diffuse-sde-27. @fig-diracdelta-27 shows the evolution of $p(x,t)$ in time.

$$
p(x,t) = \frac{1}{\sqrt{2 \pi t}} e^{-x^{2}/(2 t)}
$$  {#eq-diffuse-sde-27}

One way to describe @eq-diffuse-sde-27 is a normally distributed random variable, with $E[p(x,t)]=0$ and $\sigma^{2}$ (the variance) equal to $t$. Notice how the mean and variance for @eq-diffuse-sde-27 connect back to our previous work with random walks and diffusion in Chapters [-@sec-random-walks-23] and [-@sec-diffusion-24]. Namely, simulations and random walk mathematics showed that the expected value of a random walk or Brownian motion was zero and the variance grew in time, which is the same for this SDE.


```{r,echo=FALSE,fig.cap="Profiles for the solution to SDE $dx = dW(t)$ (given by @eq-diffuse-sde-27) for different values of $t$."}
#| label: fig-diracdelta-27
...
Applying @eq-fpe-27 to this SDE we obtain @eq-diffuse-advect-sde-27:

$$
p_{t} = -r \, p_{x}+ \frac{\sigma^{2}}{2} \, p_{xx} 
$$ {#eq-diffuse-advect-sde-27}

@eq-diffuse-advect-sde-27 is an example of a *diffusion-advection equation*.\index{diffusion-advection equation}   Amazingly through a change of variables we will reduce @eq-diffuse-advect-sde-27 to an example of @eq-diffuse-sde-27. Here's how to do this:

- First, let $x=z+r \, \tau$ and $t=\tau$. This change of variables may seem odd, but our goal here is to write $p(x,t)=p(z,\tau)$ and to develop expressions for $p_{\tau}$ $p_{z}$, and $p_{zz}$ from this change of variables. But in order to do that, we will need to apply the *multivariable chain rule* (see @fig-chain-rule).\index{chain rule!multivariable}


```{tikz,label=fig-chain-rule,warning=FALSE,message=FALSE,echo=FALSE,fig.cap='Multivariable chain rule.',fig.width=4,fig.height=3}
...
- Finally if re-write @eq-diffuse-advect-sde-27 with the variables $z$, $\tau$, and the change of variables (@eq-cov-27) we obtain @eq-diffuse-advect-cov-sde-27.
...
\end{split}
$$  {#eq-diffuse-advect-cov-sde-27}

All this work and transformation of variables to obtain @eq-diffuse-advect-cov-sde-27 yielded a diffusion equation in the variables $z$ and $\tau$!  Applying @eq-diffuse-sde-27 with the variables $z$ and $\tau$, we have $\displaystyle p(z, \tau) = \frac{1}{\sqrt{2 \pi \sigma^{2} \tau}} e^{-z^{2}/(2 \sigma^{2} \tau)}$, which we then transform back into the original variables $x$ and $t$ (@eq-diffuse-advect-soln-27):

$$
p(x, t) = \frac{1}{\sqrt{2 \pi \sigma^{2} t}} e^{-(x-rt)^{2}/(2 \sigma^{2} t)}
$$ {#eq-diffuse-advect-soln-27}

Now that we have an equation, next let's visualize the solution. Let's take a look at some representative plots in @fig-diff-advec:


```{r,echo=FALSE,fig.cap="Profiles for the solution to SDE $dx = r \\; dt + \\sigma \\; dW(t)$ (given with @eq-diffuse-advect-soln-27) for different values of $t$."}
#| label: fig-diff-advec
